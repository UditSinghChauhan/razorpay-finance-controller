# PREREGISTRATION — ASSAY Benchmark v1.0.13

**Spec version:** 1.4.39 · **Benchmark version:** 1.0.13
...
**no** metric, carrying `§9`'s literals with the bump and opening `§10` **V31**; and
2026-09-02 (benchmark 1.0.12), where **spec 1.4.35 adds a sixth entry to `§7`** —
metric 7's ECE population, binned prediction and correctness predicate — amends
`§8`'s row **7** with the ratified semantics, carries `§9`'s literals with the bump
and opens `§10` **V32**; and 2026-09-02 (benchmark 1.0.13), where **spec 1.4.36
REVISES `§7`'s metric-17 entry** — the first revision of a `§7` entry in this
corpus, adding the baseline pair's **encoding and rounding rule** and its
**source-transcription rule**, adding **no** `§7` entry and **no** metric, amending
`§8`'s row **17** with the ratified encoding, adding a **clarification only** to
`§9`'s step 0 while preserving all eight steps in number, order, command and flag,
carrying `§9`'s literals with the bump and opening `§10` **V33** — see below; and
2026-09-02 at **benchmark 1.0.13 UNCHANGED**, where **spec 1.4.37 TRANSCRIBES `§9`
step 0's measured metric-17 baseline into `§7`** and ratifies what `EVALUATION_SPEC.md
§4.10` publishes against it. Step 0 has been **taken**, returning
`mean_bps = stddev_bps = 0` for all five `offline` Tier-0 keys; `§7`'s metric-17 entry
gains a `measured`, a `deferred` and a `consequence` row and its `rate`, `population`,
`statistic`, `scope`, `producer`, `consumer`, `table`, `rounding`, `transcription` and
`k_sigma` rows are **preserved verbatim**. It adds **no** `§7` entry and **no** metric,
leaves `§8`'s list at **28** with metric 17's name, number and formula untouched and
`k_sigma` at **3**, preserves all **eight** `§9` steps in number, order, command and
flag, and opens `§10` **V34**. **The benchmark version does not move** — the precedent
being spec 1.4.31, which likewise amended no pre-registered quantity — so `§9`'s
literals and step 1's `bench-v1.0.13` tag are **carried unchanged**. **This is the
first amendment in this corpus taken after a measured figure exists**, and it says so
rather than claiming otherwise: see `DATA_MODEL.md §22.2` **M59** and
`DECISION_BRIEF.md §A.44` for the legitimacy argument; and 2026-09-17 at
**benchmark 1.0.13 UNCHANGED**, where **spec 1.4.39 opens `§10` V37 and V38 after the
seal and changes no pre-registered quantity**: V37 records a **defect in the sealed
engine** — `RECONCILIATION_SPEC.md §6`'s materiality computed as `0` where it was
undefined, so a multi-candidate settlement without an `AN2` bank line **committed**
where the oracle labelled it `TRULY_AMBIGUOUS` — found after the seal, present in the
submitted artifact, and corrected post-submission (`DATA_MODEL.md §22.2` **M61**,
`DECISION_BRIEF.md §A.46`); V38 records that `SE1`, `SE2` and `SE4` are specified and
not implemented, `SE5` is inert before a probe, and `§6`'s evidence-gap arm has never
fired. Both rows state explicitly whether any sealed v1.0.13 figure is affected (none
changes value). It adds **no** `§7` entry and **no** metric, leaves `§8`'s list at
**28**, preserves all **eight** `§9` steps, moves no threshold, and re-scores nothing;
`§9`'s literals and step 1's `bench-v1.0.13` tag are **carried unchanged**
...
**Amendment 1.4.35 / benchmark 1.0.12 (pre-seal, metric 7's correctness semantics).**
Applied before the seal, before any dataset was generated, before any agent was
scored and before any metric was computed — `bench/` absent, `runs/` holding only
`.gitkeep`, no seal tag cut. One register row, `DATA_MODEL.md §22.2` **M57**; record
at `DECISION_BRIEF.md §A.42`; threat row `§10` **V32**.

**The gap, stated exactly.** `EVALUATION_SPEC.md §4.6` froze metric 7's formula, its
ten equal-width bins, its reliability diagram and its ε-gap scope, and named
`accuracy(bin)` **without defining what makes a committed decision right**. At least
two readings stood on the frozen text and they **disagree numerically**: **set
equality** against the true allocation, which `RECONCILIATION_SPEC.md §6` and `M35`
already call *"allocation identity — the set of `(target_id, member_obs_id)` pairs
the solution asserts"*; and **edge-level agreement**, under which
`EVALUATION_SPEC.md §4.2`'s `FP` clause alone would decide and a decision asserting
a **subset** of the true members would carry no false positive and count as correct.
An outcome-bearing choice between admissible readings is a governance act taken
**before any figure exists**, never a decision an implementation makes for itself.

**What is settled.** `§7` gains a sixth entry giving metric 7's **population** —
`RECONCILIATION_SPEC.md §6` step 3's `DISCRIMINATED` branch, the one accept in which
the ε-gap decided the gate — its **binned prediction**, `Δs` in integer basis points,
its **unit**, one committed decision to one prediction, and its **correctness
predicate**, set equality of the asserted allocation against the full true member
set for the same target. `§8`'s row **7** carries the same ruling in the metric
list. `EVALUATION_SPEC.md §4.6` states it in full, including the exhaustive
treatment of subsets, supersets, an absent target and an empty allocation, and the
bin-edge and `N = 0` conventions.

**What is NOT settled and NOT touched.** `§4.6`'s formula is preserved **verbatim**,
as are its ten bins and its reliability-diagram requirement; metric 7 keeps its
**name and number**; `EVALUATION_SPEC.md §4.2` is **read and not amended**, its
edge unit, its `FP`/`FN` clauses and its abstained/excepted exclusion untouched, and
metric **5** remains the partial-credit metric. `§8`'s list stays at **28**, none
added, removed, renumbered or redefined, and **no other metric's formula, universe
or threshold moves**. `§4.1`–`§4.5` and `§4.7`–`§4.13`; `§5.1`'s ε grid and `§5.3`'s
sweeps; `§5.4`'s **thirteen** obligations and `§5.5`'s forbidden practices; `§4.2`'s
`F01`–`F12` families, their operators, rates and magnitudes; `§5`'s oracle, `§5.3`'s
two gates and their frozen draw; `§5.4`'s ambiguity definition; `§6.1`'s split and
seed table and every generation seed; and **every pre-existing threshold in `§7`** —
`τ`, `ε`, `K_max`, `C_max`, `P_max`, `C_review`, `C_exception`, `k_sigma`,
`queue_top_n`, `max_unresolved_ratio_bps`, the `SE1`–`SE5` weights, the `A3-NOLLM`
probe priority policy, the `§5.3` consistency draw, the ε and cost sweep grids, the
metric-15/16 populations, metric 15's per-case harm and the metric-17 baseline — are
unchanged. `C1`–`C8` are untouched so `constraint_set_hash` does not move. `AL1`–`AL8`
are untouched in substance **and** in wording, and `§9`'s eight steps keep their
number, order, command and flag, **step 0** included. `M51`–`M56` stand entire.

**`BENCHMARK_VERSION` moves 1.0.11 → 1.0.12.** The bump is taken on **M39**'s
precedent, as 1.0.4 → 1.0.5, 1.0.8 → 1.0.9, 1.0.9 → 1.0.10 and 1.0.10 → 1.0.11 all
were, and **not** on **M49**'s: no conforming agent's postings change. **M50**'s
non-bump is distinguished rather than overlooked — that row **withdrew** two
expectations and changed nothing a scored artifact contains, whereas this row decides
whether metric 7 is a **number** at all rather than an unavailable state, which is
**M56**'s own test applied to one figure instead of nine. `§9` step 1's tag and step 5's
literal are carried with it, on `M46`'s precedent that a bump must reach `§9`'s code
block in the same amendment that takes it; `§9`'s **step 0** is unchanged.
`SPEC_VERSION` moves **1.4.34 → 1.4.35**; `GT_VERSION` stays **1.1.0**, no
`GroundTruth` field being added, read differently or regenerated; `RunKey` stays
`(agent_id, split, seed, llm_mode)`, `RunConfig` is unchanged and `DATA_MODEL.md §18`'s
`BenchmarkManifest` **shape** stays closed. **No artifact byte changes and no dataset
exists to regenerate.** `V17` and `V22`–`V31`, `§F`'s rows and `§H`'s dispositions are
unchanged and are not reopened; `§10` **V32** is opened by this amendment. **No
implementation is taken:** `packages/eval/src/metrics/calibration.ts`,
`apps/cli/src/bench/scorer.ts` and `apps/cli/src/artifacts/metrics.ts` are all
untouched, and the build follows per `DECISION_BRIEF.md §I`. Historical amendment
records are preserved **verbatim**.

**Amendment 1.4.36 / benchmark 1.0.13 (pre-seal, metric 17's baseline encoding and
record authority).** Applied before the seal, before any dataset was generated, before
any agent was scored, before any metric was computed and — the condition that makes
this particular amendment legitimate — **before the baseline it governs was ever
measured**: `bench/` absent, `runs/` holding only `.gitkeep`, no seal tag cut,
`§9` **step 0 not taken** and `packages/eval/src/frozen.ts`'s `METRIC_17_BASELINE`
empty. One register row, `DATA_MODEL.md §22.2` **M58**; record at
`DECISION_BRIEF.md §A.43`; threat row `§10` **V33**.

**The two gaps, stated exactly.** `M53` gave metric 17 its rate, population,
statistic, scope, producer, consumer and table, and left two things open that decide
what the sealed run reports. **First, rounding.** `§7` named the fields `mean_bps` and
`stddev_bps` and stated **no rounding rule** — neither the mode, nor when it applies,
nor whether the two figures round independently, nor which values the detector reads.
Two readings were admissible on the frozen text: rounding once at the end to the
frozen integer pair, or holding full-precision values with `_bps` as a display
encoding. They **disagree numerically**, therefore on `abstention_spike_flag`.
**Second, the second representation.** `§7` is a markdown table and TEST scoring is
code; the measured baseline must exist in source for the scorer to read it, and `M53`
chartered `§7` alone. Neither gap could be closed by an implementation choosing for
itself, and neither could be closed after step 0 without the choice being made by
someone who had already seen the rates.

**What is settled — ratification 1, encoding and rounding.** `§7`'s metric-17 entry
gains a `rounding` row. `mean_bps` and `stddev_bps` are **integer basis points**. The
five per-seed rates enter the mean and the **sample** standard deviation at **full
precision** and are not rounded first. Each statistic is converted to bps and rounded
**exactly once**, at the end of `§9` step 0's arithmetic, by **`round_half_up` with
ties away from zero**. The two figures are rounded **independently**, each from its own
full-precision result, and `stddev_bps` is never re-derived from `mean_bps` or from
rounded inputs. The detector consumes the **rounded** pair —
`rate > mean_bps / 10_000 + k_sigma * stddev_bps / 10_000` — against the run's own
**full-precision** rate, with `k_sigma` **unchanged at 3**. **No second, unrounded
baseline exists anywhere in the system.** The choice rests on `DATA_MODEL.md §0`
**rule 5**, which requires an integer-bps encoding for *"every dimensionless ratio
that enters a hashed body, **or that a gate or invariant compares**"* — this pair
being the right-hand side of `§4.10`'s detector — and whose `§20` carve-out is
**enumerated, closed and conditioned** on values *"computed at render from the
authoritative integer paise fields"*, which a statistic over five prior runs is not.

**This is metric 17's rule and nothing wider.** It is **not** a claim that half-up is
the corpus's only rounding or quantization mode. `M27`'s `mode_days` **floors** — and
`M27` ratified that floor explicitly — while `EVALUATION_SPEC.md §4.6`'s bin selection
**floors**, and remainder distribution **floors**. Those rules stand **unchanged** and
are not reopened. What `M58` follows from `M27` is its **structure**: `M27` quantized
the recorded, compared term (`mode_days`) and left the continuous term continuous
(`lag_days`, *"the unfloored real quotient"*), scoping the rationale *"to the mode
alone"*; `M58` quantizes the recorded baseline pair and leaves the run's own rate at
full precision.

**What is settled — ratification 2, record authority and transcription.** `§7`'s
metric-17 entry gains a `transcription` row. **`§7` is the authoritative
human-readable baseline record.** Its **executable transcription** is
`METRIC_17_BASELINE` in `packages/eval/src/frozen.ts`, which is **intentionally empty
(`[]`) before step 0**. After step 0 measures the five DEV rates, the exact
`(agent_id, llm_mode, mean_bps, stddev_bps)` rows are transcribed into **`§7`** and
into **that constant**, **after step 0 and before `§9` step 1's tag**, so the tag
covers the source carrying the measured baseline and step 8's *"no code changes
between 6 and 8"* holds over it. The constant is `§7`'s transcription and **not a
second independently measured baseline**; **no runtime scoring may recompute the
baseline**; **any divergence between `§7` and `METRIC_17_BASELINE` is a
seal/reproducibility failure**; **no value may be guessed, prefilled or populated
before step 0**; **no generated JSON or data file** is introduced as another evidence
path, on the second-evidence-path ground `AL8`, `DECISION_BRIEF.md §A.31` and `M56`
all rest on; and the constant is **not** a `BenchmarkManifest` field, `M53` having
already rejected that and `DATA_MODEL.md §18`'s shape staying **closed**.

**The command surface and step 0's invocation — clarification, not procedure.**
`assay bench --baseline` is the **implementation spelling** of step 0's
`<non-scored DEV baseline pass>`, which names no command; it is a **mode of the
existing `bench` command and not a ninth command**, and
`DECISION_BRIEF.md §C` **T0-11**'s list of **eight** is unchanged and unedited. `§9`
step 0's *"under each `llm_mode`"* constrains the **completeness of `§7`'s table** — a
row per `(agent_id, llm_mode)` — and **not the number of process invocations**; step 0
is already written as three command lines, so the operator may take it **once per
`llm_mode`**. This adds **no step, renumbers none, and creates no second benchmark
procedure**. Where `DECISION_BRIEF.md §F` **F2** is unresolved the `replay` rows are
deferred exactly as **F2** already defers `B2-LLM-DIRECT` and metric 3's
`--llm=replay` column, and metric 17 reads **UNAVAILABLE** for the absent rows;
**F2's semantics are applied, not reopened, and its disposition is untouched.**

**What is NOT settled and NOT touched.** `§4.10`'s formula `rate > baseline + k·σ` is
preserved **verbatim**, as is `k_sigma = 3`; metric 17 keeps its **name and number**;
the **rate's universe, its numerator and its denominator** are `M53`'s and do not
move, and neither do the **population** (the five DEV seeds `2000`–`2004`, `n = 5`),
the **statistic** (mean and **sample** standard deviation), the **scope**
(`(agent_id, llm_mode)`, not pooled), the **producer** or the **consumer**. `§8`'s
list stays at **28**, none added, removed, renumbered or redefined, and **no other
metric's formula, universe or threshold moves**. Every pre-existing `§7` threshold —
`τ`, `ε`, `K_max`, `C_max`, `P_max`, `C_review`, `C_exception`, `k_sigma`,
`queue_top_n`, `max_unresolved_ratio_bps`, the `SE1`–`SE5` weights, the `A3-NOLLM`
probe priority policy, the `§5.3` consistency draw and its member-set bound, the ε and
cost sweep grids, the metric-15/16 populations and metric 15's per-case harm — is
unchanged. `C1`–`C8` are untouched so `constraint_set_hash` does not move. `AL1`–`AL8`
are untouched in substance **and** in wording, `AL5`'s emission reading included, and
`§9`'s **eight steps keep their number, order, command and flag**, **step 0**
included. `DATA_MODEL.md §0` rule 5, `§18`'s shape, `§21`'s `AbstentionTelemetry` and
`§1`'s `GroundTruth` are **read and not amended**. `M1`, `M27` and `M51`–`M57` stand
entire.

**`BENCHMARK_VERSION` moves 1.0.12 → 1.0.13.** The bump is taken on **M39**'s
precedent, as 1.0.4 → 1.0.5, 1.0.8 → 1.0.9, 1.0.9 → 1.0.10, 1.0.10 → 1.0.11 and
1.0.11 → 1.0.12 all were, and **not** on **M49**'s: no conforming agent's postings
change. **The pre-registered surface changes in what the sealed run can yield** for
`abstention_spike_flag` and for the echoed `baseline_rate_by_value` /
`baseline_stddev`, because the two admissible readings disagree numerically — which is
**M57**'s own test applied to metric 17. **M50**'s and **M45**'s non-bumps are
distinguished rather than overlooked: **M50** withdrew expectations and changed nothing
a scored artifact contains, and **M45** governed *when* the test dataset becomes
reachable; this row changes an artifact's **contents**. Under-bumping is the error
**M46** exists to make unrepeatable, so `§9` step 1's tag and step 5's literal are
carried in this same amendment. `SPEC_VERSION` moves **1.4.35 → 1.4.36**; `GT_VERSION`
stays **1.1.0**, no `GroundTruth` field being added, read differently or regenerated
and metric 17 reading no ground truth at all; `RunKey` stays
`(agent_id, split, seed, llm_mode)`, `RunConfig` is unchanged and `DATA_MODEL.md §18`'s
`BenchmarkManifest` **shape** stays closed. **No artifact byte changes and no dataset
exists to regenerate** — no manifest, run, root hash or `bench-v1.0.13` tag was ever
produced. `V17` and `V22`–`V32`, `§F`'s rows and `§H`'s dispositions are unchanged and
are not reopened; `§10` **V33** is opened by this amendment.

**This amendment REVISES a `§7` entry, which is a first, and it says so.** `M56` and
`M57` both recorded *"`§7` gains no entry and revises none"*; this row **revises**
`M53`'s metric-17 entry by adding two rules to it. That is permitted only because
**nothing has been observed**: `§L.4` bars changing a `§7` parameter *"on the basis of
an observed result"*, `§6.2` **AL3** permits pre-seal adjustment *"only on an argument
that does not reference measured performance"*, and **no rate, mean or σ exists** —
step 0 has not run. The entry specifies **how a not-yet-existing value is formed**; it
changes **no value**, because there is none to change.

**No implementation is taken:** `packages/eval/src/metrics/abstention.ts`,
`apps/cli/src/bench/baseline.ts`, `apps/cli/src/bench/scorer.ts`,
`apps/cli/src/commands/bench.ts` and `apps/cli/src/artifacts/metrics.ts` are all
untouched in behaviour, the baseline algorithm is unchanged, `METRIC_17_BASELINE`
stays `[]`, and the build follows per `DECISION_BRIEF.md §I`. **One runtime follow-up
is chartered and named here:** `packages/eval/src/metrics/abstention.ts` currently
asserts that *"Half-up is this corpus's **only** rounding mode"* and that *"nothing in
the corpus uses another"*. **That statement is false** — `EVALUATION_SPEC.md §4.6`'s
bin selection floors, `M27`'s `mode_days` floors, and remainder distribution floors —
and it is currently offered as the **justification** for the very rule this amendment
ratifies. The runtime `M58` commit **must** replace it with wording grounded in this
metric-17-specific ratification, and **must** state explicitly that other rules in this
corpus use other rounding and quantization modes. Historical amendment records are
preserved **verbatim**.
...
```
  ...
  Abstention spike detection (THREAT_MODEL.md §T9, M2):
      ...
      baseline              = the mean and SAMPLE standard deviation of
                              ...
                              are given in full below, and so are the ENCODING,
                              ROUNDING and TRANSCRIPTION rules (spec 1.4.36,
                              register row M58). k_sigma itself is UNCHANGED at
                              3 and no formula moves.
  ...
  this entry supplies what the k_sigma block above leaves open):
      ...
      rounding            RATIFIED at spec 1.4.36, register row M58; §7 named
                          the fields mean_bps/stddev_bps and stated no rounding
                          rule, and two readings were admissible.
                          mean_bps and stddev_bps are INTEGER basis points.
                          The five per-seed rates enter the mean and the SAMPLE
                          standard deviation at FULL PRECISION -- they are not
                          rounded first. Each statistic is then converted to
                          bps and rounded EXACTLY ONCE, at the end of §9 step
                          0's arithmetic, by round_half_up with ties away from
                          zero. The two figures are rounded INDEPENDENTLY, each
                          from its own full-precision result; stddev_bps is
                          never re-derived from mean_bps nor from rounded
                          inputs. The detector consumes the ROUNDED pair:
                            abstention_spike_flag =
                              rate > mean_bps / 10_000
                                     + k_sigma * stddev_bps / 10_000
                          against the run's own FULL-PRECISION rate, k_sigma
                          unchanged at 3. NO SECOND, UNROUNDED BASELINE EXISTS
                          ANYWHERE IN THE SYSTEM.
                          This is METRIC 17's rule. It is NOT a claim that
                          half-up is the corpus's only rounding or quantization
                          mode: M27's mode_days floors and EVALUATION_SPEC.md
                          §4.6's bin selection floors, and both stand unchanged.
                          Residual: §10 V33.
      transcription       RATIFIED at spec 1.4.36, register row M58.
                          §7 -- THIS TABLE -- is the AUTHORITATIVE
                          human-readable baseline record. Its EXECUTABLE
                          TRANSCRIPTION is METRIC_17_BASELINE in
                          packages/eval/src/frozen.ts, which is intentionally
                          EMPTY ([]) before step 0. After step 0 measures the
                          five DEV rates, the exact
                          (agent_id, llm_mode, mean_bps, stddev_bps) rows are
                          transcribed into THIS table and into that constant,
                          AFTER step 0 and BEFORE §9 step 1's tag -- so the tag
                          covers the measured transcription. The constant is
                          §7's transcription and NOT a second independently
                          measured baseline; NO runtime scoring may recompute
                          the baseline; ANY divergence between this table and
                          METRIC_17_BASELINE is a SEAL/REPRODUCIBILITY FAILURE;
                          and NO value may be guessed, prefilled or populated
                          before step 0. No generated JSON or data file is
                          introduced as another evidence path, and the constant
                          is NOT a BenchmarkManifest field.
      k_sigma             3, unchanged, above.

      measured            TRANSCRIBED at spec 1.4.37, register row M59. §9 step
                          0 has been TAKEN. These are its measured rows, copied
                          UNCHANGED from the pass's own table in M58's field
                          order. Nothing here is computed, re-derived, rounded a
                          second time or adjusted, and NO value was measured
                          twice:

                            agent_id       llm_mode   mean_bps  stddev_bps
                            ASSAY          offline           0           0
                            B0-IDONLY      offline           0           0
                            A1-NOVALIDATE  offline           0           0
                            A2-NOABSTAIN   offline           0           0
                            A3-NOLLM       offline           0           0

                          This table is AUTHORITATIVE. Its executable
                          transcription is METRIC_17_BASELINE in
                          packages/eval/src/frozen.ts, which carries these same
                          five rows and these four fields only; any divergence
                          between the two is a SEAL/REPRODUCIBILITY FAILURE.

      deferred            The seven (agent_id, llm_mode) keys below carry NO
                          PAIR. §7 records them HERE, with their reasons; they
                          are NOT written into METRIC_17_BASELINE, which
                          transcribes PAIRS and has no field for a reason. Each
                          reads metric17BaselineFor(...) === null, which is §7's
                          UNAVAILABLE -- never a pair of zeros and never a
                          spike_flag of false. §7 is therefore COMPLETE about
                          what it does not hold.

                            B2-LLM-DIRECT  replay     DECISION_BRIEF.md §C T0-10
                            B2-LLM-DIRECT  offline    / §F F2: this baseline is
                                                      conditional on a metered
                                                      credential to populate its
                                                      replay cache; F2 records
                                                      that credential as
                                                      Unresolved and its
                                                      pre-declared response
                                                      defers the agent to tier
                                                      H2 rather than weakening
                                                      it. §9 step 0 applies that
                                                      disposition and does not
                                                      reopen it.

                            ASSAY          replay     DECISION_BRIEF.md §F F2:
                            B0-IDONLY      replay     no recorded replay cache
                            A1-NOVALIDATE  replay     exists at this checkpoint
                            A2-NOABSTAIN   replay     (EVALUATION_SPEC.md §2
                            A3-NOLLM       replay     populates it from one
                                                      recorded --llm=<live
                                                      provider> --record pass,
                                                      which F2 leaves untaken),
                                                      so step 0's replay rows are
                                                      deferred exactly as F2
                                                      already defers
                                                      B2-LLM-DIRECT and metric
                                                      3's --llm=replay column.
                                                      NOTHING is fabricated in
                                                      their place.

      consequence         RATIFIED at spec 1.4.37, register row M59. A measured
                          (0, 0) pair IS a baseline: metric17BaselineFor returns
                          it, the flag is COMPUTED, and EVALUATION_SPEC.md §5.5's
                          unavailable-with-reason is NOT engaged -- that
                          convention governs a key §7 records NO PAIR for, which
                          is a different condition from a key whose measured pair
                          is zero. The detector is UNCHANGED and this is its
                          arithmetic, not a redefinition: on (0, 0) the frozen
                          expression
                                                      rate > mean_bps / 10_000
                                                             + k_sigma * stddev_bps / 10_000
                          evaluates to rate > 0 + 3 * 0, i.e. rate > 0. The
                          comparison being strict, a scored run carrying no
                          abstained recon_line value yields false and one
                          carrying any positive abstained recon_line value yields
                          true. WHAT THE PUBLISHED FLAG IDENTIFIES FOR SUCH A KEY
                          IS THEREFORE THE PRESENCE OF ANY POSITIVE ABSTAINED
                          recon_line VALUE, NOT AN EXCURSION ABOVE A
                          NON-DEGENERATE REFERENCE; the word "spike" in the
                          field's name is read with this row attached.
                          §4.10's expectation is met on its own terms -- a clean
                          run does not fire, an F10 run carrying abstained value
                          does -- and NO frequency, magnitude or expected firing
                          rate is asserted, no scored run existing.
                          WHY THE MEASUREMENT IS ZERO, from already-recorded
                          structure and not from a new finding: §10 V17 records
                          that every DEV settlement is fully AN1-anchored because
                          F08's DROP_SETTLEMENT_ID is TEST-ONLY, so no DEV target
                          enumerates a candidate; §10 V28 records that §6.1 makes
                          F07-F10 test-only. A population enumerating no
                          candidate presents no AMBIGUOUS or INTRACTABLE target,
                          so no recon_line reaches ABSTAINED. A2-NOABSTAIN is
                          zero on a second and independent ground, being the
                          agent EVALUATION_SPEC.md §3.2 defines as never
                          abstaining; that ground is V33's and is unchanged.
                          V33 is NOT edited and its A2-NOABSTAIN instance is not
                          withdrawn: this row records a SECOND instance, and a
                          zero MEAN besides, which V33's "the bar collapses to
                          mean_bps" does not describe.
                          REJECTED and preserved as rejected: reporting the flag
                          UNAVAILABLE for a measured (0, 0) key; re-running step
                          0; widening the DEV population (§6.1's forbidden list,
                          AL7, V28); and altering the detector, threshold or
                          formula (§6.2 AL3, DECISION_BRIEF.md §L.1 rule 12 and
                          §L.4, which forbids revising from a result).
                          Residual: §10 V34.
  ...
  Metric 7 ECE population, prediction and correctness (RATIFIED at spec 1.4.35,
  register row M57; frozen on the A3-NOLLM terms, being the semantics of a
  figure on §8's list):

      population          the scored unit's COMMITTED decisions carrying a
                          non-null score -- RECONCILIATION_SPEC.md §6 step 3's
                          DISCRIMINATED branch, the one accept in which the
                          ε-gap decided the gate. UNIQUE has no second solution
                          and therefore no gap to bin; IMMATERIALLY_AMBIGUOUS
                          was decided by the materiality clause; AMBIGUOUS
                          abstains and commits no allocation; INTRACTABLE
                          commits nothing. None of the four enters.
      prediction          ONE COMMITTED DECISION = ONE PREDICTION. N is the
                          number of such decisions in the scored unit.
      binned value        Δs = |evidence_score_bps(best)
                                - evidence_score_bps(second)|, an integer in
                          basis points -- the quantity DATA_MODEL.md §13 carries
                          as AmbiguityCertificate.evidence_score_gap_bps.
      correctness         assert(d) = { (d.target_id, e) : e a member entity
                                        of d }
                          truth(d)  = { (target_id, entity_id) : a TRUE
                                        allocation edge whose target_id
                                        = d.target_id }
                          correct(d) iff assert(d) = truth(d)   SET EQUALITY
                          This is M35's ALLOCATION IDENTITY applied as a
                          comparison rather than as a sort key. Correctness is
                          against the FULL true member set for that target,
                          never against individual truth edges.
      subset / superset   a strict subset of the true member set is INCORRECT;
                          a superset is INCORRECT; a target the truth carries no
                          edge for gives truth(d) = the empty set, so any
                          non-empty assertion against it is INCORRECT; an
                          asserted EMPTY allocation is correct only against a
                          true empty allocation (M35's (target_id, "")).
      §4.2 not imported   EVALUATION_SPEC.md §4.2's edge unit, its FP/FN clauses
                          and its "(excluding abstained/excepted)" parenthesis
                          are READ AND NOT SUBSTITUTED. Metric 5 remains the
                          partial-credit / set-membership metric.
      bins                ten equal-width bins of 1000 bps over the FULL
                          0..10_000 range, never the observed range; lower edge
                          inclusive and upper exclusive EXCEPT the tenth, which
                          includes 10_000; an empty bin contributes no term.
      N = 0               the metric is published UNAVAILABLE with its reason.
                          NEVER 0.0 (EVALUATION_SPEC.md §5.5, M56).
      rejected            calibrating evidence_score_bps itself instead of Δs;
                          including UNIQUE decisions with an invented score;
                          including IMMATERIALLY_AMBIGUOUS decisions;
                          edge-level / partial-credit correctness; the edge as
                          the prediction unit; and leaving the metric
                          unresolved. All are preserved as rejected (M57).
      formula             UNCHANGED. §4.6's
                          ECE = Σ_bins (n_bin / N) × |accuracy(bin)
                                                     - mean_score(bin)|,
                          its ten bins and its reliability diagram are not
                          touched, and metric 7 keeps its name and number.
```
...
7. `ece` — expected calibration error of the score used for abstention.
   **Correctness semantics ratified at spec 1.4.35 (register row `DATA_MODEL.md
   §22.2` M57):** the population is `RECONCILIATION_SPEC.md §6` step 3's
   **`DISCRIMINATED`** branch — the committed decisions whose gate the ε-gap
   actually decided — the binned prediction is that decision's `Δs`, **one
   committed decision is one prediction**, and a decision is **correct iff the set
   of `(target_id, entity_id)` edges it asserts equals the true allocation's set
   for the same target**. `EVALUATION_SPEC.md §4.6`'s formula, its ten
   equal-width bins and its reliability diagram are **unchanged**, and the metric
   keeps its number and its place on this list of 28. `§4.2`'s edge unit and its
   abstained/excepted `FN` exclusion are **not substituted**; metric 5 remains
   the partial-credit metric. Where the population is empty for an agent the
   metric is published **unavailable with its reason**, never `0.0`. See `§7`,
   `§10` **V32**.
8. `gap_to_oracle`
...
17. `abstention_spike_flag` — fires on the F10 split, not on clean splits.
    ...
    `rate > baseline + k·σ` and `k_sigma = 3` are unchanged. **The baseline
    pair's encoding is ratified at spec 1.4.36, register row `DATA_MODEL.md
    §22.2` M58:** `mean_bps` and `stddev_bps` are integer basis points, each
    rounded `round_half_up` (ties away from zero) **exactly once** at the end of
    step 0 from its own full-precision statistic, and the detector compares the
    run's **full-precision** rate against the **rounded** pair. `§7` remains the
    authoritative record and `packages/eval/src/frozen.ts`'s
    `METRIC_17_BASELINE` its executable transcription. **The formula, the
    universe and `k_sigma` do not move**; what M58 supplies is the encoding
    `§7` named and never stated. See `§10` **V28** and **V33**.
...
**Spec 1.4.36 / benchmark v1.0.13 dependency statement (register row `DATA_MODEL.md
§22.2` M58).** One metric is affected and it is listed here so that no reader has to
infer which number moved.

**Encoding supplied, formula untouched:** metric **17** `abstention_spike_flag` (M58,
`EVALUATION_SPEC.md §4.10`, `§7`). `M53` gave the rate its universe and the baseline
its population, statistic, scope, producer and consumer, and named the table's fields
`mean_bps` and `stddev_bps` **without stating a rounding rule** — no mode, no point of
application, no statement of whether the two figures round independently, and no
statement of which values the detector reads. Two readings were admissible and they
**disagree numerically**, therefore on the flag. `§7` now carries the encoding: integer
basis points; full-precision inputs to both statistics; `round_half_up`, ties away from
zero, applied **exactly once** at the end of `§9` step 0; the two figures rounded
**independently**; and the detector comparing the run's **full-precision** rate against
the **rounded** pair. Stating that metric 17 is "unaffected" would be false: its
baseline had no determinate encoding, and it has one now. **The formula
`rate > baseline + k·σ`, `k_sigma = 3`, the rate's numerator and denominator, the
five-seed population, the sample statistic and the `(agent_id, llm_mode)` scope are
unchanged**, and this list stays at **28**, none added, removed or renumbered.

**Definition amended:** none other. **Formula amended:** none. **Universe amended:**
none — metric 17's own universe is `M53`'s and does not move. Metrics **18** and
**19**, the other two `§4.10` figures, are **genuinely unaffected**: neither reads a
baseline. Metric **7** keeps `M57`'s semantics, metric **5** remains the partial-credit
metric, metric **4** scores against `oracle_labels.jsonl`, and metric **10** stays
`NOT COMPUTABLE ON THE FROZEN POPULATION` per **M54**. `M51`'s grids, `M52`'s
populations, `M55`'s per-case harm, `M56`'s three states and `M57`'s correctness
predicate are untouched.

**Availability.** Where `§7`'s table records **no row** for a scored unit's
`(agent_id, llm_mode)` — which is its state until `§9` step 0 has run, and which
includes the `replay` rows wherever `§F` **F2** stays unresolved — metric 17's **flag**
is published **UNAVAILABLE with its reason** and never `false`. A detector reporting
*"no spike"* against a baseline it does not have is the broken detector
`EVALUATION_SPEC.md §4.10` names, and a `false` from an absent baseline is the
fabricated number `§5.5` bars. The **rate** is published on every split regardless: it
is `§4.10`'s input and a property of the run alone. **No population size, frequency,
baseline value or expected value is asserted anywhere in this amendment: no dataset
exists and step 0 has not run.**

**Spec 1.4.35 / benchmark v1.0.12 dependency statement (register row `DATA_MODEL.md
§22.2` M57).** One metric is affected and it is listed here so that no reader has to
infer which number moved.

**Semantics supplied, formula untouched:** metric **7** `ece` (M57,
`EVALUATION_SPEC.md §4.6`). `§4.6` froze the formula, the ten equal-width bins, the
reliability diagram and the ε-gap scope, and named `accuracy(bin)` **without defining
what makes a committed decision right** — at least two readings were admissible and
they disagree on a decision asserting a subset of the true members. `§7` now carries
the population, the binned prediction, the unit and the correctness predicate.
Stating that metric 7 is "unaffected" would be false: its correctness term had no
determinate value, and it has one now. **The formula, the bin count, the reliability
diagram, the metric's name and its number are unchanged**, and this list stays at
**28**, none added, removed or renumbered.

**Definition amended:** none other. **Formula amended:** none. **Universe amended:**
none other — metric **5** `match_precision` / `match_recall` / `match_f1` is
**genuinely unaffected**, `EVALUATION_SPEC.md §4.2` being read and not amended in its
edge unit, its `FP`/`FN` clauses or its abstained/excepted exclusion, and it remains
the metric that reports partial credit. Metric **4** is unaffected, scoring against
`oracle_labels.jsonl`. Metric **10** stays `NOT COMPUTABLE ON THE FROZEN POPULATION`
per **M54**; **M54**'s disposition does not transfer to metric 7, which has a truth
axis whose definition was merely undetermined. `M51`'s grids, `M52`'s populations,
`M53`'s baseline, `M55`'s per-case harm and `M56`'s three states are untouched.

**Availability.** Where the `§4.6` population is empty for an agent — an agent whose
gate consults no score — metric 7 is published **unavailable with its reason** and
never `0.0`, which is `M56`'s own rule and `EVALUATION_SPEC.md §5.5`'s bar on a
number that does not exist in a committed run artifact. **No population size,
frequency or expected value is asserted anywhere in this amendment: no dataset
exists.**
...
```
  0. Baseline:     assay generate --split dev --seeds 2000-2004
     ...
     # ENCODING (spec 1.4.36, M58): the five rates enter the mean and SAMPLE
     #   stddev at FULL PRECISION; each statistic is converted to bps and
     #   rounded ONCE here, round_half_up, ties away from zero, the two figures
     #   INDEPENDENTLY. §7's `rounding` row is the rule.
     # TRANSCRIPTION (spec 1.4.36, M58): the measured rows are written into §7
     #   AND into packages/eval/src/frozen.ts's METRIC_17_BASELINE -- §7 is the
     #   authoritative record, the constant its executable transcription -- and
     #   BOTH are written HERE, before step 1's tag, so the tag covers them.
     #   Divergence between the two is a SEAL FAILURE. Before this step the
     #   constant is intentionally EMPTY and NOTHING may be prefilled.
     # ONE CONCEPTUAL STEP, ONE INVOCATION PER llm_mode (spec 1.4.36, M58):
     #   "under each llm_mode" constrains the COMPLETENESS OF §7's TABLE -- a
     #   row per (agent_id, llm_mode) -- and NOT the number of process
     #   invocations; step 0 is already written as three command lines. The
     #   operator may therefore take it once per llm_mode. This adds NO step,
     #   renumbers none, and introduces no second benchmark procedure. Where
     #   DECISION_BRIEF.md §F F2 is unresolved the `replay` rows are deferred
     #   exactly as F2 already defers B2-LLM-DIRECT and metric 3's replay
     #   column, and metric 17 reads UNAVAILABLE for the absent rows; F2's
     #   semantics are APPLIED, not reopened.
  ...
  1. Freeze code:  git tag -s bench-v1.0.13 -m "ASSAY benchmark v1.0.13 seal"
  ...
  2. Generate:     assay generate --split test --seal-tag bench-v1.0.13 \
  ...
  5. Commit hashes into bench/test/<seed>/benchmark_manifest.json, ONE PER (split, seed)
     ...
     # `benchmark_version` must read "1.0.13" (DATA_MODEL.md §18)
...
**The transcription rides inside step 0, and that placement is ratified (spec 1.4.36,
register row `DATA_MODEL.md §22.2` M58).** Step 0 does not end when the arithmetic
ends; it ends when both representations of the measured baseline exist — `§7`'s table,
which is the **authoritative human-readable record**, and
`packages/eval/src/frozen.ts`'s `METRIC_17_BASELINE`, which is that table's
**executable transcription** and nothing else. Both are written **before step 1**, so
the tag covers the source that carries the measured baseline and step 8's *"no code
changes between 6 and 8"* holds over it. Writing the constant **after** the tag would
put a measured figure outside the seal; recomputing it at scoring time is refused by
`§7`'s consumer row; and a **divergence between the table and the constant is a seal /
reproducibility failure**, on the same footing as step 5's other seal failures. The
ordering is therefore **generate DEV → step 0 (measure → record into `§7` → transcribe
into source) → step 1 → steps 2–6 → step 7**.
...
| V32 | Metric 7's correctness predicate is **set equality on the allocation**, which is **not equivalent** to the edge-level reading `EVALUATION_SPEC.md §4.2` uses for metric 5, and its population is the **accept side of the very threshold ε that calibration exists to justify** | **Declared at spec 1.4.35 (register row M57), not repaired, because neither half can be repaired without reopening a ratified reading.** **The predicate.** `§4.6` named `accuracy(bin)` and defined it nowhere; set equality and edge-level agreement were both admissible and they disagree on a decision asserting a **subset** of the true members, which set equality counts **wrong** and edge-level agreement counts **right**. The consequence is that `accuracy(bin)` is weakly lower than an edge-wise reading would give and `ece` is generally a different number, so **metric 7 is not comparable with any externally computed edge-wise calibration figure** and no such comparison may be drawn in the report. **This is a disclosure, not a defect:** the gate's claim is that *this allocation explains this target*, and partial credit is reported where `§4.2` put it — **metric 5** `match_precision` / `match_recall` / `match_f1`, whose definitions this amendment does not touch. **The population.** Only `RECONCILIATION_SPEC.md §6` step 3's `DISCRIMINATED` decisions enter, because they are the only ones whose gate the ε-gap decided; an `AMBIGUOUS` decision **abstains** and has no committed allocation for `accuracy(bin)` to test. Metric 7 therefore observes the **accept** side of ε alone, and **miscalibration below ε is structurally unobservable to it** — `EVALUATION_SPEC.md §4.3`'s metrics 4 and `§4.13`'s metric 8 are where the abstain side is scored, against the oracle. **Sparse or empty bins are a structural property of that population, never grounds for a definition change.** Bins below ε cannot be populated by construction, and an agent whose gate consults no score contributes no prediction at all, so its cell reads **unavailable with its reason** rather than `0.0` (`§5.5`, `M56`). Widening the population to restore density would mean including `UNIQUE` decisions with an **invented** score or `IMMATERIALLY_AMBIGUOUS` decisions whose gate consulted no gap — both **rejected and preserved as rejected** at `M57` — and `§6.2` **AL3** with `DECISION_BRIEF.md §L.4` forbid revisiting the choice once a figure has been seen. **No population size, frequency or expected value is claimed here:** `bench/` is absent, `runs/` holds only `.gitkeep`, no dataset has been generated and no metric has been computed, so the statement is structural rather than empirical. **Fixed before any figure exists** — which is what makes it legitimate, on `M55`'s and `M56`'s own condition |
| V33 | Metric 17's `abstention_spike_flag` compares a **full-precision rate** against a **quantized bar**, so a rate lying within roughly **2 bps** of the true full-precision bar may fall on the opposite side of the flag from where unrounded arithmetic would place it; and where an agent's five DEV rates agree to within `5e-5`, `stddev_bps` rounds to **0** and the bar collapses to `mean_bps` | **Declared at spec 1.4.36 (register row M58), not repaired, because the alternative is worse and the residual is the price of an auditable record.** `§7`'s table names `mean_bps` and `stddev_bps`, and `DATA_MODEL.md §0` rule 5 requires an integer-bps encoding for *"every dimensionless ratio that … a gate or invariant compares"* — which this pair is, being the right-hand side of `§4.10`'s detector. Rule 5's carve-out cannot be reached: it is **enumerated and closed** (*"the four `coverage_by_value*` figures and `coverage_by_count` in `CloseReport` (§20)"*) and it is **conditioned** on being *"derived display values computed at render from the authoritative integer paise fields"*, while the baseline is a statistic over five runs that no longer exist as artifacts when the flag is evaluated. **The quantization band is bounded and stated:** `round_half_up` moves each figure by at most `0.5` bps, so the bar `mean_bps + 3·stddev_bps` moves by at most `2` bps against a rate that is not rounded at all. **The `σ = 0` case is real and is not a rounding artifact alone:** an agent that never abstains — `A2-NOABSTAIN` is the declared instance — has five identical DEV rates and a **true** sample σ of exactly zero, so the collapsed bar is the detector behaving correctly on a degenerate population, not a defect introduced here; what M58 adds is the narrower band `0 < σ < 0.5 bps` in which a genuinely non-zero σ records as `0`. **The rejected alternative carried a worse residual.** Full-precision internal values with `_bps` as display-only would relocate the rounding decision into the `§7` transcription and leave it **unspecified** there, since a markdown table cannot carry a double without a decimal-place convention; and it would make `M58`'s own companion rule — *"any divergence between `§7` and `METRIC_17_BASELINE` is a seal failure"* — **unenforceable**, because no exact comparison exists between a truncated rendering and a full-precision literal. An integer-vs-integer check is one a reviewer can actually perform, and that is what the disclosure buys. **`M27`'s precedent is followed, not contradicted:** that row quantized the **recorded, compared** term (`mode_days`, floored) and left the continuous term continuous (`lag_days`, *"the unfloored real quotient"*), scoping the rationale *"to the mode alone"*; M58 quantizes the recorded baseline pair and leaves the run's own rate at full precision, which is the same shape. **`V28` is unchanged and rides beside this row:** `n = 5` already bounds what a `3σ` bar can resolve, and the family-composition boundary already confounds a fired flag; this residual is **additional to** those and repairs neither. **No figure is claimed:** `bench/` is absent, `runs/` holds only `.gitkeep`, no dataset has been generated, no baseline has been measured and no flag has been evaluated, so the band is stated **structurally** and no frequency, magnitude or expected effect is asserted. **Fixed before any figure exists**, which is what makes it legitimate, on `M55`'s, `M56`'s and `M57`'s own condition: `§6.2` **AL3** and `DECISION_BRIEF.md §L.4` are satisfied rather than merely not engaged |
| V34 | Metric 17's DEV baseline measured at `§9` step 0 is **`(0, 0)` for every Tier-0 agent under `offline`**, so `EVALUATION_SPEC.md §4.10`'s bar collapses to `0` for all of them and the published `abstention_spike_flag` distinguishes *any abstained value* from *none* rather than resolving a `3σ` excursion; the flag's power to discriminate a graded abstention increase is therefore not merely low, as **V28** already records for `n = 5`, but **absent by construction** on this baseline | **Declared at spec 1.4.37 (register row M59), not repaired, because both repairs are foreclosed by the frozen corpus and the second is foreclosed twice.** **Repair 1 — widen the DEV population so a non-degenerate baseline can be measured.** Rejected, and already rejected once: **V28** records that *"widening the baseline population would require generating `F07`–`F10` into DEV, which `§6.1`'s forbidden list bars and which would destroy the family-level holdout `V3` depends on"*, and `§6.2` **AL7**'s burn rule attaches to any breach of that list. **V17** records the same structure from the oracle's side — *"every DEV settlement is fully `AN1`-anchored"*, `F08`'s `DROP_SETTLEMENT_ID` being test-only — and disposes of it the same way, *"accepted, and recorded before the seal"*. **Repair 2 — floor the bar, impose a minimum `σ`, relax `>` to `≥`, or move `k_sigma`.** Rejected on two independent grounds. First, `k_sigma = 3` is frozen in `§7` and bound by `§6.2` **AL3**; `M58` preserved `§4.10`'s formula *"VERBATIM"*; `DECISION_BRIEF.md §L.1` rule 12 lists the threshold among the constants no composition root may respell. Second, and dispositively, the change would be made **in response to a measured figure**, which `DECISION_BRIEF.md §L.4` forbids — *"forbids revising it from a result"* — and would leave the detector tuned on its own output. **The distinction this row turns on:** a detector whose usefulness is bounded by its population is a **disclosure**; a detector adjusted because its output was seen is a **defect in the procedure**. This row takes the first and refuses the second. **What is bounded, not eliminated:** on the sealed run a fired flag remains confounded by **V28**'s family-composition boundary and now carries, in addition, no separation between a single abstained paisa and a flood. **What still carries the magnitude:** `abstention_rate_by_value` is published beside the flag on **every** run and is a property of that run alone, unaffected by the baseline; it is the quantity a reader must use to judge size, and `EVALUATION_SPEC.md §4.3`'s metric 4 and `§4.13`'s metric 8 remain where the abstain side is scored against the oracle. **No figure beyond the measured baseline is claimed:** no scored run exists, `runs/` holds only `.gitkeep`, no TEST seed has been generated or inspected, no flag has been evaluated on any scored unit, and this row asserts **no frequency, magnitude or expected effect** for the sealed run. **Unlike `V28`–`V33`, this row is opened AFTER a measured figure exists**, and it does not claim the *"fixed before any figure exists"* condition those rows close on; the legitimacy argument that replaces it is stated in full at `DATA_MODEL.md §22.2` **M59** and rests on the figure having come from the already-frozen procedure, on nothing here tuning or revising it, on no scored TEST result having been observed, and on no threshold or benchmark parameter moving |
| V35 | **The sealed TEST population exercises none of the ambiguity, abstention or validation machinery this benchmark exists to compare.** On all **50** scored units of `§9` step 7's run — five agents × ten seeds under `llm_mode = offline` — `truly_ambiguous = 0`, `abstentions = 0` and `probes_spent = 0`; metric 17's `abstention_rate_by_value` is `0` and `abstention_spike_flag` is `false` on every unit; metric 3's `aurc_paise` is `0` and metric 7's `ece` is `null` with `M57`'s `N = 0` reason, on every unit. The ε sweep's **21** points and the τ sweep's **4** points are **flat** on all **20** curve units — identical `coverage_by_value` and identical `balance_harm_paise` at every ε from `0` to `10_000` bps, identical `coverage_by_value` and identical outcome counts at every τ floor from ₹10 to ₹10,000 — with `DISCRIMINATED = 0` and `AMBIGUOUS = 0` at **every** swept point. `ASSAY`, `A1-NOVALIDATE`, `A2-NOABSTAIN` and `A3-NOLLM` are **numerically identical on every reported figure** across the whole clean block `9000`–`9004`, where `B0-IDONLY` matches them too; they remain identical to each other across the held-out block `9100`–`9104`. The **only** material separation anywhere in the run is `ASSAY` against `B0-IDONLY` on `9100`–`9104` | **Declared at spec 1.4.37 after the sealed run, as a DISCLOSURE of what was measured, and NOTHING is repaired, re-run or revised.** **What this row is:** documentation of a measured population limitation. **What it is not:** a benchmark failure, a defect in the seal, or a post-hoc modification. `§9`'s eight steps were executed in order, every oracle completeness gate passed on all ten TEST seeds, all ten manifests satisfy step 5's conditions, and the run exited `0` and wrote 50 conforming artifacts; the procedure did what it says. **The measured cause, and it is already on this list rather than newly discovered.** **V17** records that *"every DEV settlement is fully `AN1`-anchored"* and that *"the completeness gate passes on DEV without ever enumerating a candidate"*; **V28** records that `§6.1` makes `F07`–`F10` test-only. A population on which no candidate is enumerated presents no target `RECONCILIATION_SPEC.md §6` can mark `AMBIGUOUS` or `INTRACTABLE`, so `§9`'s abstention branch is never reached and no `recon_line` reaches `ABSTAINED`. **What this run adds is that the same structure holds on TEST, including the held-out block** — the oracle labelled **0** targets ambiguous on all ten TEST seeds, `1,580 NO_SOLUTION` and `1,535 UNAMBIGUOUS` and nothing else — which **V34**'s premise had left open when it rested metric 17's `(0, 0)` baseline on a DEV-only reading of V17. `V34` is not edited and its instance is not withdrawn; this row records the wider consequence it did not reach. **What is therefore NOT supported by this run, stated plainly:** the ablations `A1-NOVALIDATE` and `A2-NOABSTAIN` remove mechanisms that this population never engages, so their identity with `ASSAY` is **evidence about the population and not about the mechanisms**. `EVALUATION_SPEC.md §3.2` calls the ablations *"the scientific controls"*; on this corpus they control nothing, because the treatment they remove was never applied. **The middle clause of this section's own entitled-claim block — *"removing its deterministic validation layer measurably increases rupee-denominated financial error"* — is NOT supported by the sealed run and must not be asserted from it.** It is not contradicted either: it is **untested**, which is a different and weaker statement, and the honest form of both is that the sealed population cannot discriminate the abstention or validation ablations at all. **The one separation, reported with its direction rather than only its existence:** on `9100`–`9104` `ASSAY` reaches higher `coverage_by_value` than `B0-IDONLY` on every seed (`1.0000`/`0.8182`, `1.0000`/`0.8097`, `0.9956`/`0.7210`, `0.9986`/`0.8094`, `0.9639`/`0.7473`) while `B0-IDONLY` records **lower** `balance_harm_paise` and **lower** `net_cost_paise` on every seed — `net_cost_paise` being `§4.5`'s *"single comparable figure"*. That direction is recorded here because `§5.5` bars reporting accuracy without coverage and this row would otherwise state a separation while concealing which way it runs. **No aggregate and no interval is claimed anywhere in this row.** `EVALUATION_SPEC.md §5.2`'s bootstrap is not implemented at this checkpoint, so no cross-seed mean, no ± 95% CI and no CI-overlap verdict exists for any figure; every number above is a **per-scored-unit** value read from a committed `metrics.json`, which is the only form `§5.5` permits. `--llm=replay` was not run — `§F` **F2** stays unresolved — so metric 24 `offline_parity` has no second column and is unavailable rather than zero. **Four alternatives are rejected and preserved as rejected:** **re-running step 7**, the run being deterministic over sealed inputs, so a re-run returns identical values and spends clean provenance for nothing; **regenerating TEST under a different composition, family mix or degradation rate** so that ambiguity appears, which would be a change made **in response to a measured result**, forbidden by `DECISION_BRIEF.md §L.4` and requiring a new `BENCHMARK_VERSION` and fresh seeds under `§6.2` **AL3**, with `AL7`'s burn rule attached to the seeds already generated; **moving τ, ε, `k_sigma` or the `AN1` anchor rule** to make the machinery engage, barred on the same two grounds and by `M58`'s verbatim preservation of `§4.10`; and **withholding or re-labelling the measured zeros**, which `§5.5` forbids outright and which would make the artifact less honest than the run. **What is unchanged by this row:** every formula, threshold, population, family assignment, seed, split, rate and metric definition; `§8`'s list at **28**; `RunKey`, `RunConfig` and `GroundTruth`; the signed tag `bench-v1.0.13`, the step-6 seal point, the ten `BenchmarkManifest` files, the TEST corpus and all 50 `metrics.json`, none of which is touched, re-scored or amended. **This row is opened AFTER a scored result exists** and, like `V34`, does not claim the *"fixed before any figure exists"* condition `V28`–`V33` close on; its legitimacy is that it reports the measurement exactly as produced by the already-frozen procedure, changes no rule in response to it, and withdraws the claim the result does not support instead of preserving one the data cannot carry |
| V36 | **`PROJECT_SPEC.md §7` S3 is NOT met on the sealed TEST corpus, and the measured cause is an interaction between two frozen clauses rather than a defect in ASSAY's posting behaviour.** On all ten TEST seeds under `llm_mode = offline`, `ASSAY`'s `balance_harm_paise` is **1.2305×–1.4954×** `batch_value_paise` against S3's bar of **0.05%** — three orders of magnitude, not a marginal miss. `EVALUATION_SPEC.md §4.4(a)`'s `proj_truth` selects `true_journal` lines *"whose `source_entity_id` belongs to a covered observation"*, and `DATA_MODEL.md §1` gives **one** `pay_…` key to both the `P1` capture leg and the `P2` bank leg of the same payment. A `recon_line` enters the covered set on an allocation that needs no bank-side evidence, while `DATA_MODEL.md §17.1.1` conditions `P2`/`P4` on *"`AN2` satisfied against an actual `bank_line`"* — and `§4.2` freezes `bank_ref` quality at *"30% a clean UTR, 70% absent or non-UTR"*, so `AN2` is unavailable on most settled value **by construction**: **54 of 186** settlements carry an `AN2`-matchable bank line on each of `9000`–`9004` and **35–36 of 124** on `9100`–`9104`, which is **V19**'s `realize(30/100, 31) = 9` per family instance exactly. `proj_truth` therefore charges a conforming agent, in full, the bank leg `§17.1.1` forbids it to post | **Declared after the sealed run, as a DISCLOSURE of what was measured, and NOTHING is repaired, re-run, re-scored or revised.** **What this row is:** documentation of a measured scope interaction between two clauses that are each correct against their own frozen text. **What it is not:** an ASSAY posting-rule defect, a defect in the seal, or a post-hoc modification. **The measured signature, reproducible from committed artifacts alone.** On all **50** scored units `by_account["4000_REVENUE"]` and `by_account["9000_SUSPENSE_UNRECONCILED"]` are **0**, and on all ten seeds two identities hold **exactly**: `balance_harm_paise = 2 × by_account["1100_GATEWAY_RECEIVABLE"]`, and `by_account["1100_…"] = by_account["1200_BANK"] + by_account["1300_…"] + by_account["2200_…"] + by_account["5100_…"]`. That is the algebraic fingerprint of *agent posted `P1`/`P3`, truth posted `P1`+`P2` / `P3`+`P4`*, since `amount = credit + (fee − tax) + tax`; revenue agrees because `P2`/`P4` do not touch `4000_REVENUE`. **The `P2` path is exercised and works where its evidence exists, which is what forecloses the posting-defect reading.** Summing truth's `P2`/`P4` `1100` legs over every allocated entity whose settlement carries **no** `AN2`-matchable `bank_ref` predicts `by_account["1100_…"]` **exactly to the paise** on `9100`–`9104`, and to within **0.59%–2.57%** on `9000`–`9004`, that residual being the allocation entities which reached `EXCEPTION` rather than `RECONCILED` and so leave **both** projections (`covered_entities` 3651 against 3713 truth allocations on seed 9000). On seed 9000 the agent's covered `1200_BANK` is **1,140,385,609** paise, which is the sum of the `P2`/`P4` bank legs on `AN2`-anchored settlements to the paise: the agent posts the bank leg on precisely the ~21% of value where `AN2` holds, and withholds it elsewhere as `§17.1.1` requires. **The corpus already carried every premise and never composed them.** **V16**, **V18** and **V19** each derive consequences of the same 30/70 `bank_ref` share — for `C3`'s bank-arrival half, for metric 27 and the completeness gate, and for `S12`'s `CLOSED` half — and stop short of `§4.4`; `§17.1.1` closes its own trigger paragraph with *"This does not depress recon-view coverage … with metric 27 `coverage_by_value_bank` carrying the exposure"*, naming the metric it believed carried the consequence; `RECONCILIATION_SPEC.md §11`'s worked example states *"this example is an `F08` case with no `AN2` match, so `P2`/`P4` do not fire"*; and `§8`'s benchmark v1.0.3 dependency statement runs the identical argument for a **missing `P1`** — *"an agent that omitted `P1` would have carried harm of roughly `batch_value_paise` against S3's 0.05% bar"*. The symmetric argument for `P2`/`P4` was never run. This row runs it, from the measurement. **What is therefore NOT supported by this run, stated plainly:** `balance_harm_inr` on this corpus measures the frozen benchmark's **bank-attribution rate**, not ASSAY's accounting accuracy, and **no claim about accounting accuracy, trial-balance trustworthiness or financial harm may be drawn from it**. Because `EVALUATION_SPEC.md §4.5` makes `balance_harm_inr` the first term of metric 2 `net_cost_inr`, `§5.1` makes it metric 3 `aurc_paise`'s y-axis, and `§4.13` makes metric 8 `gap_to_oracle` a difference of two `net_cost_inr` figures, **metrics 2, 3 and 8 inherit this limitation in full and may not be presented as independent evidence of accounting accuracy on this corpus either.** **V35**'s reported direction — `B0-IDONLY` recording lower `balance_harm_paise` and lower `net_cost_paise` on `9100`–`9104` — is **not** withdrawn and **not** re-read: it stands as measured, and this row records that the quantity it runs on is dominated by the structural term described above. `S2` is unaffected and is met; `coverage_by_value` reads `0.9639`–`1.0000` for `ASSAY` on the ten seeds, and its universes are unchanged. **Four alternatives are rejected and preserved as rejected:** **relaxing `§17.1.1`'s `AN2` condition on `P2`/`P4`, or adding an amount- or date-based bank anchor**, which would assert that money arrived on evidence that does not establish it — the failure `I5`'s own purpose column names, *"Claiming money arrived that did not"* — and would be an engine change made in response to a measured result, forbidden by `DECISION_BRIEF.md §L.4`; **re-scoping `§4.4`'s `proj_truth`, or re-keying truth's `P2`/`P4` lines onto a distinct `source_entity_id`**, which is a metric-definition and `GroundTruth`-schema change after a figure exists, barred by `§6.2` **AL3** and `§L.4` and requiring a new `BENCHMARK_VERSION` and fresh seeds with **AL7**'s burn rule attached; **regenerating TEST at a higher clean-`bank_ref` share** so the interaction stops binding, barred on the same two grounds and identically to the third alternative **V35** rejects; and **moving S3's threshold, or restating `balance_harm_inr` against the covered value rather than `batch_value_paise` to make the ratio smaller**, which `DECISION_BRIEF.md §F` **F9** forbids outright — *"the threshold may NOT be adjusted in response to what the check shows"* — and which would make the artifact less honest than the run. **The repair that is available, and it is not available now.** A future benchmark may raise `§4.2`'s clean-`bank_ref` share, or give truth's `P2`/`P4` legs a join key `§4.4`'s covered-set projection can separate from `P1`/`P3`. Either is a composition or schema change requiring a **new `BENCHMARK_VERSION`**, fresh seeds under **AL3**, and **AL7**'s burn rule against the seeds already generated; neither is taken here and neither is designed here. **What is unchanged by this row:** every formula, threshold, population, family assignment, seed, split, rate and metric definition; `§8`'s list at **28**; `§4.4`'s `proj_agent`, `proj_truth` and Suspense exclusion; `§17.1.1`'s trigger table and `§17.1`'s `P1`–`P8`; `AN1`–`AN4`; `RunKey`, `RunConfig` and `GroundTruth`; the signed tag `bench-v1.0.13`, the step-6 seal point, the ten `BenchmarkManifest` files, the TEST corpus and all 50 `metrics.json`, none of which is touched, re-scored or amended. **No aggregate and no interval is claimed:** `§5.2`'s bootstrap is not implemented at this checkpoint, so every figure above is a **per-scored-unit** value read from a committed `metrics.json` or recomputed from a committed `bench/test/<seed>` artifact, which is the only form `§5.5` permits. **This row is opened AFTER a scored result exists** and, like **V34** and **V35**, does not claim the *"fixed before any figure exists"* condition **V28**–**V33** close on; its legitimacy is that it reports the measurement exactly as produced by the already-frozen procedure, changes no rule in response to it, and reports a success criterion **failed** rather than preserving a claim the data cannot carry |
| V37 | **The sealed engine COMMITTED, rather than abstained, on any multi-candidate settlement without an `AN2`-matched bank line — a defect found after the seal, present in the submitted artifact, and fixed post-submission at spec 1.4.39.** Through spec 1.4.38 `packages/engine/src/s4-solve.ts`'s `balances()` returned an **empty** projection when `bank_evidence === null`; `materiality()` then computed the difference of two empty projections as **`0`**; `0 ≤ τ` held; `solve` returned **`IMMATERIALLY_AMBIGUOUS`**; and `apps/cli`'s `classifyTarget` **committed `ranked[0]`**. `§4.2` freezes `bank_ref` quality at *"30 % a clean UTR, 70 % absent or non-UTR"*, so this was the engine's behaviour on the **majority** of any population of targets with two or more admissible allocations — while `§5.4`'s oracle, whose `projectAllocation` posts `P2`/`P4` unconditionally, labels the same target **`TRULY_AMBIGUOUS`**. An absent comparand was licensing a commit, on exactly the permissive reading `DATA_MODEL.md §17.1.1` rejects for `I5` (*"undefined — not satisfied — when no bank-line mapping exists"*). The thesis that under-determined evidence produces a typed abstention did not hold on that population under the sealed engine | **Declared at spec 1.4.39 after the sealed run, as a DISCLOSURE of a defect in the submitted engine, and NOTHING in the sealed artifact is repaired, re-run, re-scored or revised.** **What this row is:** documentation of a code defect in the engine that produced `runs/seal-v1.0.13`, stated with its pre-fix behaviour, and of the correction applied **after** submission — `DATA_MODEL.md §22.2` **M61**, `RECONCILIATION_SPEC.md §6`'s new ladder row, `DECISION_BRIEF.md §A.46`. **What it is not:** a change to any sealed byte, threshold, population, metric or seed. **How it was found.** By reading the code during the design of a successor benchmark (`docs/BENCH_V2_DESIGN.md §1.1`), not by any measurement: the sealed run never reached the defective branch, so no figure could have revealed it. **The correction, stated.** Materiality is **undefined — `null`, not `0`** — without a comparand; the `IMMATERIALLY_AMBIGUOUS` branch is evaluated only when materiality is defined, skipped rather than passed when it is not; a multi-candidate component with undefined materiality abstains with a fifth certificate reason, **`MATERIALITY_UNDETERMINED`**, on a different axis from the four search-failure reasons and implying a different human action (obtain the bank line). Scope: fires only on `≥ 2` admissible candidates with `bank_evidence === null` and `Δs < ε`; a single candidate is still `UNIQUE` and commits; `DISCRIMINATED` stays reachable. **No threshold moves**: `τ`, `ε`, `K_max`, `C_max`, `P_max` and the `SE1`–`SE5` weights are byte-identical. `SPEC_VERSION` moves 1.4.38 → 1.4.39 so a run of the corrected engine names itself in `engine_commit`; `BENCHMARK_VERSION` **stays 1.0.13** because a bump would name a sealed run that does not exist, and the corrected engine's first sealed run belongs to a successor benchmark under fresh seeds. **Runs either side of this row are NOT comparable.** **Whether any sealed v1.0.13 number is affected, stated explicitly: no sealed figure changes value, and one sealed reading gains a second cause.** The oracle labelled **0** targets `TRULY_AMBIGUOUS` and **0** `IMMATERIALLY_AMBIGUOUS` on all ten TEST seeds — every enumerated target had at most one solution (V35: `1,580 NO_SOLUTION`, `1,535 UNAMBIGUOUS`) — so the defective branch (`≥ 2` candidates **and** no bank evidence) was **never entered** on any of the 50 scored units; every `metrics.json` would be byte-identical under the corrected engine, and `abstentions = 0` is a fact about the population, not a symptom of this defect. What changes is the **reading** of V35's identity `ASSAY = A2-NOABSTAIN`: V35 attributes it to the population alone, and that attribution stands; this row adds that even on a population that *did* pose the question, the sealed engine would have equalled `A2-NOABSTAIN` on every multi-candidate target without a clean `bank_ref` — about 70 % of them under `§4.2` — because it committed there too. **The entitled-claim block below is qualified accordingly:** its first clause, *"ASSAY's abstention policy is calibrated against an ambiguity oracle"*, is **not supported by the sealed engine** on the non-`AN2` population, where that engine's policy and the oracle's label systematically disagreed; it is supported by the corrected engine and remains untested on any sealed corpus (V35). **The demo fixture is unaffected in its headline case and affected in its surroundings:** `demo-500`'s abstained settlement carries an `AN2`-matched bank line, so its certificate was produced by the measured-materiality path and stands as captured; any other `demo-500` settlement with two admissible allocations and no bank line would, under the corrected engine, abstain where it previously committed — the demo figures in `README.md` are re-captured against the corrected engine and each before/after is reported there. **Four alternatives rejected and preserved as rejected:** **re-scoring `runs/seal-v1.0.13` under the corrected engine**, which would produce identical numbers and spend the seal's provenance for nothing; **presenting the fix as having been in the sealed engine**, which `§5.5`'s traceability forbids and this row exists to prevent; **withholding the pre-fix behaviour**, which would make the artifact less honest than the code; and **treating the defect as a threshold or definition problem** — `τ` is not moved and `§5.4` is not redefined, because the oracle's label was already right and only the engine's reading of an absent comparand was wrong. **This row is opened AFTER a scored result exists** and, like V34–V36, does not claim the *"fixed before any figure exists"* condition; its legitimacy is that the correction is a **derivation from a rule this document's companions already state** (`§17.1.1`'s `I5`), that it references no observed figure, and that the sealed artifact is reported exactly as the sealed engine produced it |
| V38 | **Of the five soft-evidence signals `RECONCILIATION_SPEC.md §4.2` specifies, `SE1` (3,500 bps), `SE2` (2,000) and `SE4` (1,000) are `const … = 0` in the engine — specified, not implemented — and `SE5` (2,000) is `0` on every candidate before a `fetch_settlement_recon` probe, which no recorded run has ever dispatched. Only `SE3` (1,500) is computed. Pre-probe the evidence score IS `SE3`, its largest gap is 469 bps against `ε = 1,500` (spec 1.4.13), and `§6`'s `DISCRIMINATED` branch — the evidence-gap arm of the decision ladder — is structurally unreachable and has never fired on any run, sealed or otherwise** | **Declared at spec 1.4.39 after the sealed run, as a DISCLOSURE of what the sealed engine computes, and NOTHING is repaired, re-run, re-scored or revised.** **What this row is:** the implementation status of a specified design, stated from the code (`RECONCILIATION_SPEC.md §4.2` "Implementation status" is the canonical statement and every other document now points to it). **What it is not:** a retirement — the rows and weights stay under `AL3`, exactly as `§4.1`'s `C8` precedent requires, and each signal keeps the register vocabulary already assigned to it: `SE1` *inactive* (1.4.10), `SE2` and `SE4` *expected-non-binding on v1.0.0 data* (1.4.20, 1.4.11), `SE5` *live, post-probe only* (1.4.15–1.4.17). What is new is saying the three things together and drawing the consequence. **The consequence, stated plainly:** the sealed engine's five-signal model was, in effect, a **one-signal model bounded below `ε` by construction**. Every `evidence_score_gap_bps` any certificate has ever carried is an `SE3` difference — `0` where two allocations' members share a settlement instant — and is small **because 6,500 of the 10,000 weighted bps are constants and 2,000 more are zero pre-probe**, not because a measured tie was found between two informative scores. **What the engine therefore abstains on is admissibility and materiality** — more than one allocation admissible under `C1`–`C8`, materially different in the books, or (from V37) that difference undeterminable — and every thesis statement in this corpus is restated that way at spec 1.4.39 (`README.md`, `ARCHITECTURE.md §3`, `PROJECT_SPEC.md` header, `DATA_MODEL.md §11`, `EVALUATION_SPEC.md` header, the engine's own comments and the certificate UI's copy). Evidence scoring ranks candidates; it does not decide whether to abstain. **Whether any sealed v1.0.13 number is affected, stated explicitly: no sealed figure changes value; one sealed figure gains its correct explanation; and one sealed flatness gains a second cause.** *(i)* Metric **7** `ece` is `null` with `M57`'s `N = 0` on all 50 units. V35 reads that off the empty population; this row adds that the population of metric 7 — `DISCRIMINATED` decisions — would have been empty on **any** pre-probe population, since the branch cannot fire before a probe and no probe ran (`probes_spent = 0`, V35). The `null` is correct twice over and is not a scorer gap. *(ii)* The **ε sweep** is flat at all 21 points on all 20 curve units. V35 attributes this to `DISCRIMINATED = 0` and `AMBIGUOUS = 0` at every point because no target had two candidates; that stands, and this row adds that on a population with two-candidate targets the sweep would still have been flat **above 469 bps**, because no pre-probe `Δs` can exceed that bound. *(iii)* `CommittedDecision.score_bps` is `null` on every committed decision in the sealed run — correctly, since `M57` makes non-null the `DISCRIMINATED` population test — so no sealed calibration input exists. *(iv)* No other metric in `§8`'s 28 references an evidence score; coverage, harm, net cost, match, close-loop, robustness and operational figures are unaffected in value and in reading. **What this row does NOT do:** implement `SE1`, `SE2` or `SE4`; wire the probe surface so `SE5` can fire; move any weight; or reallocate the 6,500 bps. Each of those is a separate amendment, and the first two are recorded as work the design specifies and the engine does not yet perform. **Two alternatives rejected and preserved as rejected:** **deleting the three rows** from `§4.2`, which would present a partly-implemented design as a complete one by subtraction; and **renormalising the two live weights to 10,000 bps**, which `AL3` bars and which would change every future `Δs` in response to an observation about the engine. **This row is opened AFTER a scored result exists** and does not claim the *"fixed before any figure exists"* condition; its legitimacy is that it states the code as it is, references no observed figure as a reason to change anything, and changes nothing |

...
**V34**. **No threat row is added and none is amended:**
`§T9` **M1**–**M6** are untouched and `M2`'s detector keeps its formula and its
`k_sigma`; the collapsed bar M59 records is a **residual disclosure** belonging to
`PREREGISTRATION.md §10`, not a new threat class — the convention `V28`, `V30`, `V31`,
`V32` and `V33` all follow.
...
`DECISION_BRIEF.md §A.43` and `PREREGISTRATION.md §10` **V33**. **No threat row is
added and none is amended:** `§T9` **M1**–**M6** are untouched, `M2`'s detector keeps
its formula and its `k_sigma`, and the quantization band M58 introduces is a
**residual disclosure** belonging to `PREREGISTRATION.md §10`, not a new threat
class — the convention `V28`, `V30`, `V31` and `V32` all follow.

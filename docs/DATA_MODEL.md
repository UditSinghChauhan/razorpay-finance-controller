...
**At spec 1.4.39** this document adds register row **M61** (`§22.2`) and widens exactly
one union: `§13`'s certificate `reason` gains a **fifth** member,
**`MATERIALITY_UNDETERMINED`**. M61 records the defect it corrects — `RECONCILIATION_SPEC.md
§6`'s materiality was computed as `0` on a target with no `AN2` bank line, so the
immateriality branch **committed** where the oracle labelled `TRULY_AMBIGUOUS` — and the
rule it applies, `§17.1.1`'s own *"`I5` is undefined — not satisfied — when no bank-line
mapping exists"*. `materiality_paise` keeps its shape; `reason` is the discriminator. No
sealed artifact is touched (`PREREGISTRATION.md §10` V37). No threshold moves.

**At spec 1.4.38** this document added register row **M60** (`§22.2`) and widens exactly
one union: `§19`'s `LlmProviderId` gains a **fifth** member, **`gemini`**, appended after
`openai-compatible`. **No other field is added, renamed or retyped and no shape changes.**
The widening is forced rather than chosen: `ARCHITECTURE.md §6.5` scopes
`openai-compatible` to *"any endpoint speaking the OpenAI chat-completions schema"*, a
native `@google/genai` call speaks Google's own, and `§19`'s `provider` field exists so
that *"a report can always state exactly what produced each decision"* — so the fourth id
was available only on a false description of the transport. **Nothing already written
moves**: no committed artifact carries the new member, this union is read for validation
and is not hashed, and every scored run records `offline`. `meteredCost` is `true` for the
new row regardless of any free tier, so `apps/cli` refuses it exactly as it refuses
`anthropic` and `openai-compatible`, `§L.1` rule 10's `--llm=offline` guarantee is
untouched and `DECISION_BRIEF.md §C` T0-11's clean checkout still runs. **No frozen
threshold, decision parameter, population, metric or line of scoring code is altered and
no observed result is referenced**, so `PREREGISTRATION.md §6.2` **AL3** and `§L.4`'s bar
on result-driven revision are satisfied rather than merely not engaged. `§18`, `§20`,
`§21` and `§1`'s `GroundTruth` are **read and not amended**, so `GT_VERSION` stays
**1.1.0**. `M1`, `M27`, `M35`, `M42`, `M46`, `M49`, `M51`–`M59` are unchanged and
preserved verbatim. **Benchmark stays v1.0.13.** See `§22.2` **M60** and
`DECISION_BRIEF.md §A.45`.

**At spec 1.4.37** this document adds register row **M59** (`§22.2`) and **does not
move** `§18`'s `benchmark_version` annotation, which stays **`"1.0.13"`**. **No field is
added, renamed or retyped and no shape changes.** `PREREGISTRATION.md §9` **step 0** has
been taken and returned `mean_bps = stddev_bps = 0` for all five `offline` Tier-0 keys;
the row ratifies **one** outcome-bearing choice that `M53` and `M58` between them left
open — **what `EVALUATION_SPEC.md §4.10` publishes when `§7` carries a measured `(0, 0)`
row**. Frozen: a measured `(0, 0)` pair **is a baseline**, so the flag is **computed**
rather than reported UNAVAILABLE, `§5.5`'s unavailable-with-reason governing a key `§7`
records **no pair** for and not a key whose measured pair is zero. The detector
consequence is stated as arithmetic and not as a redefinition: on `(0, 0)` the unchanged
expression evaluates to `rate > 0 + 3 · 0`, so what the published flag identifies for
such a key is **the presence of any positive abstained `recon_line` value**, not an
excursion above a non-degenerate reference. **The result is entailed by structure this
register already carries** — `PREREGISTRATION.md §10` **V17**'s fully `AN1`-anchored DEV
population, `F08`'s `DROP_SETTLEMENT_ID` being test-only, and **V28**'s test-only
`F07`–`F10` — and is **not** a newly discovered property of the benchmark. **`§4.10`'s
formula is preserved verbatim and `k_sigma` stays 3**; `M53`'s rate, population,
statistic, scope, producer and consumer and `M58`'s encoding, rounding and transcription
rules are unchanged; `§0` rule 5, `§18`'s **shape**, `§21`'s `AbstentionTelemetry` and
`§1`'s `GroundTruth` are **read and not amended**, so `GT_VERSION` stays **1.1.0**.
`V33` is **not edited** and its `A2-NOABSTAIN` instance is not withdrawn. `M1`, `M27`,
`M35`, `M42`, `M46`, `M49`, `M51`–`M58` are unchanged and preserved verbatim. **Benchmark
stays v1.0.13.** **This row is taken after a measured figure exists** and does not claim
otherwise — see `§22.2` **M59** for the legitimacy argument. See `DECISION_BRIEF.md
§A.44` and `PREREGISTRATION.md §10` **V34**.

**At spec 1.4.36** this document adds register row **M58** (`§22.2`) and carries
`§18`'s `benchmark_version` annotation to **`"1.0.13"`** on `M46`'s precedent. **No
field is added, renamed or retyped and no shape changes.** The row ratifies **two**
outcome-bearing choices `PREREGISTRATION.md §7`'s metric-17 entry left open. **First,
the baseline pair's encoding:** `mean_bps` and `stddev_bps` are **integer basis
points**, the five per-seed rates enter the mean and **sample** standard deviation at
**full precision**, each statistic is converted to bps and rounded **exactly once** at
the end of `§9` step 0 by `round_half_up` with ties away from zero, the two figures are
rounded **independently**, and the detector compares the run's **full-precision** rate
against the **rounded** pair — `§0` **rule 5** requiring an integer-bps encoding for
*"every dimensionless ratio that … a gate or invariant compares"*, and its `§20`
carve-out being enumerated, closed and conditioned on values *"computed at render from
the authoritative integer paise fields"*, which this pair is not. **This is metric 17's
rule and not a claim that half-up is the corpus's only rounding or quantization mode:**
`M27`'s `mode_days` floors and `EVALUATION_SPEC.md §4.6`'s bin selection floors, and
both stand unchanged. **Second, the record relationship:** `PREREGISTRATION.md §7`
remains the **authoritative human-readable** baseline record and
`packages/eval/src/frozen.ts`'s `METRIC_17_BASELINE` is its **executable
transcription**, empty before step 0, transcribed after step 0 and **before** `§9` step
1's tag, never recomputed at scoring time, and **not** a `BenchmarkManifest` field —
`§18`'s shape stays **closed**, as `M53` already rejected. Divergence between the two
is a **seal/reproducibility failure**. `§0` rule 5, `§18`'s **shape**, `§21`'s
`AbstentionTelemetry` in every field, type and comment, and `§1`'s `GroundTruth` are all
**read and not amended**, so `GT_VERSION` stays **1.1.0**. `M1`, `M27`, `M35`, `M42`,
`M46`, `M49`, `M51`–`M57` are unchanged and preserved verbatim. Benchmark moves to
**v1.0.13**. See `DECISION_BRIEF.md §A.43` and `PREREGISTRATION.md §10` **V33**.

**At spec 1.4.35** this document adds register row **M57** (`§22.2`) and carries
`§18`'s `benchmark_version` annotation to **`"1.0.12"`** on `M46`'s precedent. **No
field is added, renamed or retyped and no shape changes.** The row supplies
`EVALUATION_SPEC.md §4.6`'s missing correctness semantics for **metric 7 `ece`**:
the population is `RECONCILIATION_SPEC.md §6` step 3's **`DISCRIMINATED`** branch,
the binned prediction is that decision's ε-gap `Δs` — the quantity `§13` already
carries as `AmbiguityCertificate.evidence_score_gap_bps` — and a committed decision
is correct **iff its allocation identity equals ground truth's for the same
target**, which is `M35`'s own term applied as a comparison rather than as a sort
key. **The `§4.6` formula, its ten bins and its reliability diagram are unchanged.**
`§13`'s `Decision` and `AmbiguityCertificate` are **read and not amended** in any
field, type or comment, as are `§11`'s `Candidate.evidence_score_bps`, `§12`, `§16`,
`§17.1.1` and `§10.1`; `§1`'s `GroundTruth` is untouched, its `allocations` and
`bank_mappings` supplying the truth side through the projection the scorer already
holds, so `GT_VERSION` stays **1.1.0** and no field is added, read differently or
regenerated. `M35`, `M49`, `M52`–`M56` and `§18`'s manifest **shape** are unchanged.
Benchmark moves to **v1.0.12**. See `DECISION_BRIEF.md §A.42` and
`PREREGISTRATION.md §10` **V32**.

**At spec 1.4.34** this document adds register row **M56** (`§22.2`) and carries
`§18`'s `benchmark_version` annotation to **`"1.0.11"`** on `M46`'s precedent. **No
field is added, renamed or retyped and no shape changes.** The row rules that
`PREREGISTRATION.md §6.2` **`AL5`** is an **emission** rule — *"refuses to print, log
or write any ground-truth field; only aggregate metrics are emitted"* — so the
**scorer** may read `ground_truth.jsonl` during `§9` step 7's sealed run, which
`EVALUATION_SPEC.md §2` has always required of a scored unit on both splits. `§1`'s
`GroundTruth` is **untouched** in every field, type and comment, and its exclusion
paragraph is unchanged and is the reason the ruling is a clarification rather than a
new permission: `AL1` and `AL2` bind `packages/engine` and `packages/oracle` **by
name**, and that paragraph names engine, oracle, agent, baseline and ablation — the
scorer is none of them. `GT_VERSION` stays **1.1.0**, no field being added, read
differently or regenerated; `§16`'s `source_entity_id`, `§12`/`M28`'s relation,
`§17.1.1`'s trigger table, `§13`, `§10.1`, `§21` and `§18`'s manifest **shape** are all
unchanged. Benchmark moves to **v1.0.11**. See `DECISION_BRIEF.md §A.41` and
`PREREGISTRATION.md §10` **V31**.

**At spec 1.4.33** this document adds register row **M55** (`§22.2`) and carries
`§18`'s `benchmark_version` annotation to **`"1.0.10"`** on `M46`'s precedent. **No
field is added, renamed or retyped and no shape changes.** The row settles the one
gap `M52` left behind it: `M52` supplied metrics 15 and 16's two **populations** and
said in terms that *"the formulas in `§4.8` are unchanged; what is supplied is the
universe"*, which left metric 15's **numerator** — *"injected cases with
`balance_harm > 0`"* — with no per-case quantity to test, because
`EVALUATION_SPEC.md §4.4(a)` defines `balance_harm_inr` as a **run-level aggregate**
whose absolute value sits outside the per-account difference and therefore does not
decompose. **M55** ratifies one per-case decomposition, keyed by `§16`'s
`source_entity_id` through `§12`/`M28`'s relation, and ratifies the structural zero
for a case that posts no line. `§1`'s `GroundTruth` is untouched and **no field is
added**, so `GT_VERSION` stays **1.1.0**; `§18`'s `BenchmarkManifest` **shape** is
unchanged; `§21`'s `AbstentionTelemetry` is unchanged. **No entity, account, posting
rule, trigger, exception class, invariant definition or metric definition changes** —
`§17.1`'s `P1`–`P8`, `§17.1.1`'s trigger table, `§15`'s fourteen classes, `§16`'s
hashed body and `source_entity_id` grammar, `§14.1`'s value table and `§10.1`'s
classification are all **read** and none is amended, and `M52`'s two populations are
preserved **verbatim and unnarrowed**. `C1`–`C8` are untouched so
`constraint_set_hash` does not move; **`BENCHMARK_VERSION` moves 1.0.9 → 1.0.10**, an
input to a figure on `PREREGISTRATION.md §8`'s list entering the pre-registered
surface on `M39`'s precedent. **No artifact byte changes**, `bench/` being absent, and
**no implementation is taken**: `packages/eval/src/metrics/robustness.ts` is untouched
and metric 15 stays unwired. `M42`–`M54` are preserved verbatim. See
`DECISION_BRIEF.md §A.40` and `PREREGISTRATION.md §10` **V30**.

**At spec 1.4.32** this document adds register rows **M51**–**M54** (`§22.2`),
carries `§18`'s `benchmark_version` annotation to **`"1.0.9"`** on `M46`'s precedent,
and restates two comment lines in `§21` recording that
`AbstentionTelemetry.baseline_rate_by_value` and `baseline_stddev` **echo**
`PREREGISTRATION.md §7`'s frozen pair rather than being computed per run
(*"rolling"* retired by **M53**). **No field is added, renamed or retyped and no
shape changes.** They settle the four evaluation-procedure gaps: the ε/τ/cost
sweep contract (**M51**), metrics 15 and 16's two populations (**M52**), metric 17's
rate and baseline (**M53**), and metric 10's status as **not computable on the frozen
population** (**M54**). **No field is added, renamed or retyped.** `§1`'s
`GroundTruth` is untouched — **M52** and **M54** both turn on the fact that it
carries no partner and no cause field, and neither adds one, so `GT_VERSION` stays
**1.1.0**; `§18`'s `BenchmarkManifest` **shape** is unchanged and **M53**'s baseline
is recorded in `PREREGISTRATION.md §7` rather than in a manifest field; `§21`'s
`AbstentionTelemetry` is unchanged, **M53** supplying the universe of the
`abstention_rate_by_value` field it already declares. **No entity, account, posting
rule, trigger, exception class, invariant definition or metric definition changes** —
`§17.1`'s `P1`–`P8`, `§17.1.1`'s trigger table (cited by **M54** as running from class
to posting rather than the reverse), `§15`'s fourteen classes, `§16`'s hashed body,
`§14.1`'s value table and `§10.1`'s classification are all read and none is amended.
`C1`–`C8` are untouched so `constraint_set_hash` does not move; **`BENCHMARK_VERSION`
moves 1.0.8 → 1.0.9**, four inputs to figures on `PREREGISTRATION.md §8`'s list
entering the pre-registered surface on `M39`'s precedent. **No artifact byte
changes**, `bench/` being absent. `M42`–`M50` are preserved verbatim. See
...
interface AmbiguityCertificate {
  ...
  materiality_paise: Paise;        // max |balance_a − balance_b| over accounts;
                                   ...
                                   // the reason is the discriminator (M61)
  ...
  reason: "EVIDENCE_TIE" | "SEARCH_BOUND_EXCEEDED" | "PROBE_BUDGET_EXHAUSTED"
        | "NO_USEFUL_PROBE_AVAILABLE"    // fourth member, spec 1.4.25
        | "MATERIALITY_UNDETERMINED";    // fifth member, spec 1.4.39 (M61)
}
```

**`MATERIALITY_UNDETERMINED` is the fifth member, added at spec 1.4.39
`[ASSAY-MODEL]`, register row M61, and it is on a different axis from the four
before it.** Those four each say why the *search* failed to separate two
allocations. This one says why `RECONCILIATION_SPEC.md §6`'s *immateriality
escape hatch* was unavailable: the target has no `AN2`-matched bank line,
`§17.1.1` conditions `P2`/`P4` on that line, so neither allocation's projection
carries a bank leg and `§6`'s materiality has **no comparand** — undefined, not
satisfied, on the rule `§17.1.1` states for `I5`. Through spec 1.4.38 the engine
computed that absence as `0` and committed the top candidate. The human action it
implies is *obtain the bank line*, not *adjudicate the evidence*, which is why it
is not folded into `EVIDENCE_TIE`. It takes precedence over the attempts-derived
reasons. `materiality_paise` keeps its non-nullable shape so `§16`'s hashed body
and `CERTIFICATE_KEYS` do not move; the field reads `0` under this reason and
`reason` is the discriminator, exactly as it already was for
`SEARCH_BOUND_EXCEEDED`. **No sealed event carries the value**: `runs/seal-v1.0.13`
records zero abstentions, so every sealed `certificate` is `null` and a wider enum
parses every sealed body byte-for-byte as before.

**`NO_USEFUL_PROBE_AVAILABLE` was added at spec 1.4.25 `[ASSAY-MODEL]`, register
row M40.** It is the **fourth** member of this union — the final one on the search-failure axis; the fifth, `MATERIALITY_UNDETERMINED`, sits on the materiality axis (spec 1.4.39, M61, above) — and it closes
...
```
  ...
  materiality undefined            MATERIALITY_UNDETERMINED (spec 1.4.39, M61;
    (no AN2 bank line), any                                  precedence over the
    attempts, Δs < ε                                         three attempts rows)
```
...
interface BenchmarkManifest {
  benchmark_version: string;       // "1.0.13" (spec 1.4.36, M58; §9 step 5
                                   //  pins it at 1.0.13)
  created_at: UnixSeconds;
...
```ts
type LlmProviderId = "offline" | "replay" | "anthropic"
...
**`gemini` is the fifth member, added at spec 1.4.38 (register row `§22.2` M60),
and it is appended rather than inserted.** It exists because the fourth member
could not honestly carry it: `ARCHITECTURE.md §6.5` scopes `openai-compatible` to
*"any endpoint speaking the OpenAI chat-completions schema"*, and a native
`@google/genai` call speaks Google's own — so recording one under that id would
have made this very field misdescribe the transport that produced the answer.
The widening is **additive and changes no existing record**: no committed
artifact carries the new member, this union is read for validation rather than
hashed, and nothing already written re-hashes.
...
interface AbstentionTelemetry {
  ...
  baseline_rate_by_value: number;      // the FROZEN DEV baseline mean read from
                                       //  PREREGISTRATION §7 (spec 1.4.32, M53);
                                       //  echoed here, never computed per run
  baseline_stddev: number;             // its SAMPLE stddev, same source
...
| M51 | **The ε/τ/cost sweep contract.** The `EVALUATION_SPEC.md §5.1` ε grid is `{0, 500, …, 10_000}` bps — **21 uniform points, `1500` among them** — frozen in `PREREGISTRATION.md §7`, swept for `ASSAY` and `A1` only, under **`--llm=offline`** with metric 3's replay column deferred to `DECISION_BRIEF.md §F` **F2**. A sweep point is an evaluation **inside one scored unit**, identified `(RunKey, parameter_name, parameter_value)` and written into that unit's own `metrics.json`; **`RunKey` stays `(agent_id, split, seed, llm_mode)`**. ε and τ are `apps/cli` `bench`'s and re-execute the **agent**; `C_review`/`C_exception` are the scorer's and re-execute nothing. `tau_sensitivity` reports `coverage_by_value`, `count(AMBIGUOUS)` and `count(IMMATERIALLY_AMBIGUOUS)` at each of the four τ **floors**, and the **oracle is not re-run**. `C_review` **and `C_exception` move together** over ₹100/₹250/₹1,000 (`EVALUATION_SPEC.md §2`, `§5.1`, `§5.3`, `PREREGISTRATION.md §7`, `§8`, spec 1.4.32) | **Derived — the sweep point is not a run:** **M48** derives the scored unit as *"exactly four fields"*, `EVALUATION_SPEC.md §7` says the bootstrap *"resamples `seed` and holds the other three fixed"*, and `§5.4` item 5 with `§5.5` makes `aurc_inr` one CI-bearing scalar per unit — three clauses a fifth key dimension would each falsify. `§4.7`'s *"a separate scaling run"* is the contrast: this corpus creates a separate operation where it wants one, and creates none here. **Derived — the three execution depths:** `RECONCILIATION_SPEC.md §6` step 3 reads ε and the τ floor inside stage `S4`, so both re-execute the agent; `§4.5` and `§4.3` read the cost parameters in the scorer alone, and `§10.3`'s close policy is a ratio of **value**, not cost. **Derived — the oracle is not re-run:** `RECONCILIATION_SPEC.md §6.1` states what this sweep exists to show — *"raising τ moves cases from `AMBIGUOUS` to `IMMATERIALLY_AMBIGUOUS` … `EVALUATION_SPEC.md §5.3` reports a τ sensitivity sweep **for exactly this reason**"* — and all three quantities are engine-side; τ reaches the oracle only through `PREREGISTRATION.md §5.4`, which feeds **metric 4**, and metric 4 is not swept. **Derived — `1500` is on the grid:** `§5.2` and `§5.4` item 5 report the curve's own two axes at the frozen ε, so a grid omitting it publishes a figure the reported run cannot be located on. **Ratified — the step:** a uniform `s` must divide `10_000` and `1500`, so `s | gcd = 500` and the coarsest is **500**; uniformity is the only free choice. **Ratified — offline:** ε moves which probes fire and so the cache keys, `§2` populates the cache from *"one recorded … `--record` pass"*, and F2 leaves none; offline reaches no cache and `§2` requires it of every configuration anyway, so this **applies** F2 rather than reopening it. **Resolved a CONTRADICTION — the cost sweep:** `§5.3`'s row header and `§8`'s metric name say `C_review` alone, while `§4.5` (*"`C_review` **and** `C_exception` are assumptions… A sensitivity sweep … is mandatory"*, *"the two move together"*), `§8` twice (*"metric 26's cost sweep **scales that term**"*, which a fixed additive constant cannot do) and `DECISION_BRIEF.md §E` item 2 (*"both frozen, **both swept at ₹100 / ₹250 / ₹1,000**"*) say both. Both is adopted: it is the only reading leaving no clause vacuous, and `§8`'s rows 5, 6, 21, 22, 25 and 26 each name two or more quantities on one line, so an under-inclusive label there is that section's own form. **Rejected:** a scale factor applied to the frozen values, which no clause states and which would be invention; a fifth `RunKey` dimension; and a separate non-scored sweep mode, which `§5.5`'s *"committed **run** artifact"* rule forecloses. `C_exception`'s ₹500 is deliberately off-grid — the cost row delivers `§5.3`'s stability verdict, not a curve locating a run |
| M52 | **Metrics 15 and 16's two populations.** `injected` = observations appearing in a `GroundTruth.degradations` record whose `op` is **`INJECT_NOTES` or `CONFLICT_REFERENCE`**; `matched clean control` = observations in the **same `(split, seed)` dataset**, of an `Observation.kind` present in that dataset's injected set, appearing in **no** `degradations` record. Population reading, not bijection; **TEST seeds `9100`–`9104` only**, reported *"not exercised on DEV"* elsewhere; **no `GroundTruth` field is added** (`EVALUATION_SPEC.md §4.8`, `PREREGISTRATION.md §7`, `§8`, `§10` **V27**, spec 1.4.32) | **Derived — which operators:** `PREREGISTRATION.md §4.3`'s frozen operator→family table assigns `INJECT_NOTES` and `CONFLICT_REFERENCE` to **`F10`**, which `§4.1` alone calls *"Adversarial metadata"*; the other exercised operators are `F08`'s (*"Bank narration corruption"*) and `F04`'s (*"Duplicate bank credit"*) — an export defect and a bank behaviour. Reading *"injected"* as *"degraded"* is foreclosed by `§4.8`'s own gloss, *"no LLM output is numeric and `I6` rejects unknown IDs"* being defences against planted text and forged identifiers and neither engaged by a truncated narration. **Derived — population not bijection:** `forced_abstention_rate` is a difference of two **rates**, and a rate needs a set. **Derived — TEST-only:** `§6.1` makes `F07`–`F10` test-only and assigns `F10` to seeds `9100`–`9104`; `THREAT_MODEL.md §T9` M6 already scoped the metric to *"the sealed adversarial split"*. **Derived — no GT field is needed:** the injected set is `degradations` filtered, and its complement is the rest of the dataset. **Ratified — what *"matched"* adds:** one dataset holds seed, period, generation parameters and the agent constant by construction, and `Observation.kind` is forced on top of that because a control of a kind that can never reach `ABSTAINED` (`§17.1.1`) puts a structural zero into a subtracted rate. **Rejected:** a per-record partner field in `GroundTruth`, which would move `GT_VERSION` and make the truth artifact serve the scorer; and any covariate matching on amount, method or capture day, which no clause states — the residual is declared at `§10` **V27** instead. The formulas in `§4.8` are **unchanged**; what is supplied is the universe, on the benchmark-v1.0.3 precedent that supplied metric 13's item partition |
| M53 | **Metric 17's rate and baseline.** `abstention_rate_by_value = Σ recon_line.amount over recon_line observations whose component reached `ABSTAINED` / batch_value_paise`. The baseline is the **mean and sample stddev over the five DEV seeds `2000`–`2004`**, keyed **per `(agent_id, llm_mode)`**, produced by `PREREGISTRATION.md §9`'s new **step 0** — a non-scored pre-seal DEV pass that emits no `metrics.json` — and recorded in `§7`. TEST **reads** it; no baseline is computed at scoring time (`EVALUATION_SPEC.md §4.10`, `PREREGISTRATION.md §7`, `§8`, `§9`, `§10` **V28**, spec 1.4.32) | **Derived — the rate's universe:** `EVALUATION_SPEC.md §4.1`'s four constraints — computable from observations alone, agent-independent, each economic event once, rupee-denominated — select `batch_value_paise` unchanged, and `§4.9` already uses it as the close denominator; a numerator over all observations against it is unbounded above, and both sides over all observations reproduce verbatim the defect `§4.1` corrected, *"a quantity that can exceed unity is not a coverage rate"*. An open-Suspense-item universe is refused because `§17.1.1` gives seven classes no item. **Derived — not pooled:** `§4.10` says *"one that fires nowhere is a broken detector"*, and `A2-NOABSTAIN` never abstains while `ASSAY` does, so a pooled σ makes the flag structurally non-firing; and a shared baseline would make one agent's flag depend on another's behaviour, against `§2`'s *"attributable to the agent alone"*. `llm_mode` is separated on the same ground, `R3` probes resolving abstentions. **Derived — TEST reads a frozen value:** `§7`'s *"computed before the seal"* has no work to do under any other reading, and a detector deriving its baseline from the run it judges would never fire. **Ratified — the population:** *"rolling"* named a window this benchmark has no axis for, a `(split, seed)` dataset being one period and seeds not being ordered in time; the five declared DEV seeds are the only computable reading of *"over the DEV split"*. **Ratified — step 0 and its number:** steps 1–8 are cross-referenced by number across this corpus, so the new step takes the one number that was free. **Why `DECISION_BRIEF.md §L.4` is not engaged:** it bars **changing** a `§7` parameter *"on the basis of an observed result"*, i.e. selecting a value to obtain an outcome; here population, statistic, scope and formula are fixed **before** the measurement and the numbers are a total function of that declaration admitting no choice at computation — the property `AL7`'s successor rule and the `A3-NOLLM` policy's *"lexicographically smallest"* were written for, and the form `M44`'s consistency draw already takes, *"a frozen seed over a free sampler is vacuous"*. **Rejected:** a `BenchmarkManifest` field, `DATA_MODEL.md §18`'s shape staying closed |
| M54 | **Metric 10 `exception_class_confusion` is NOT COMPUTABLE on the frozen population**, and is reported as such with its reason. It **keeps its number** and `PREREGISTRATION.md §8`'s list stays at **28**. The computable half — the marginal distribution of R2's assigned classes — is published `EXPLORATORY` and supports no claim of triage accuracy (`EVALUATION_SPEC.md §5.4` item 5, `§6`, `PREREGISTRATION.md §8`, `§10` **V29**, spec 1.4.32) | **Derived — there is no truth side:** `§1`'s `GroundTruth` declares `gt_version`, `seed`, `family_id`, `allocations`, `bank_mappings`, `ledger_mappings`, `true_journal`, `true_balances` and `degradations`, and none is an exception class. **Derived — no frozen table supplies the mapping:** `PREREGISTRATION.md §4.3` maps operators to **families**, `§15` maps a class to its **meaning**, `§10.1` maps kinds to terminal states, and `§17.1.1` runs the other way — `PREREGISTRATION.md §8`: *"`§17.1.1` selects a *posting* from a class, **never a class from a posting**"*. **Derived — one cannot be constructed:** six exercised operators cannot cover fourteen classes; `E01`, `E02`, `E04`, `E10`, `E11`, `E12` and `E13` arise from the **true state**, which `§4.3` puts beyond every operator — *"applied to observations only, never to the true state"*; the relation is one-to-many and state-dependent, `DUPLICATE_ROW` reaching `E08` or `E09` and `MANGLE_UTR(TRUNCATE)` reaching `E14` only where a collision is realized; a degraded record often reaches **no** exception, `DROP_SETTLEMENT_ID` being designed so *"a soft path survives"*; and `DegradationRecord.target_id` and `§17.1.1`'s `setl_…`/`pay_…`/`bnk_…`/`adj_…` are different key spaces. **Ratified — publish the honest state:** the corpus's settled disposal is to publish a degenerate quantity with its explanation and never amend a definition to compensate (metric 28's structural `0.0` at `EVALUATION_SPEC.md §4.1`, metric 11 at `§10` V19, `M41`'s non-discriminating `R3` arm), and `M50` fixed the rule that *"an expectation the frozen text cannot admit is withdrawn, not reported"*; a fabricated matrix would be exactly the number `§5.5` forbids. **Rejected, and preserved as rejected:** a `GroundTruth` cause field, which couples the generator to the engine's classification — the defect `PREREGISTRATION.md §5.1`/`§5.2` exist to prevent and `§10` **V1** names — and moves `GT_VERSION`; a specification-side derived mapping, which carries that coupling and cannot be built for the reasons above; and a narrowed matrix universe, which needs the same realization tests and would choose its universe by what happened to be derivable. None may be adopted later without reopening **V1** explicitly |
| M55 | **Metric 15's per-case `balance_harm`, and the structural zero.** `injection_financial_success_rate` counts a harm **per injected observation**, not a share of a run-level figure. For injected observation `o` of `M52`'s population, `case_balance_harm(o) = Σ over AccountCode (excluding Suspense) \| proj_agent_o(acct) − proj_truth_o(acct) \|`, where both projections are `EVALUATION_SPEC.md §4.4(a)`'s, **each restricted to the journal lines whose `source_entity_id` equals `o`'s own business identifier** (`§16`, through `§12`/`M28`'s relation), and `§4.4(a)`'s covered-set scope and Suspense exclusion apply unchanged. An injected observation of a **reference kind** (`§10.1`), or whose business identifier falls outside `§16`'s `source_entity_id` grammar `pay_… \| rfnd_… \| adj_… \| setl_… \| bnk_…` — an `order_…` — contributes **`0`** and **remains in the denominator**. **`M52`'s two populations are not narrowed, metric 16 is not touched, and no `GroundTruth` field is added.** The per-case quantity **does not sum to** `§4.4(a)`'s run-level `balance_harm_inr`, and is published with `PREREGISTRATION.md §10` **V30** attached (`EVALUATION_SPEC.md §4.4`, `§4.8`, `PREREGISTRATION.md §7`, `§8`, `§10` **V30**, spec 1.4.33) | **Ratified — the decomposition, and the record says so.** `§4.8` reads *"`\|`injected cases with `balance_harm > 0``\|` / `\|`injected cases`\|`"*, which requires a harm **per case**; `§4.4(a)` defines only a **run-level aggregate**, and its absolute value sits **outside** the per-account difference and is taken over the whole covered set at once, so the aggregate does not decompose — `\|a₁+a₂ − t₁−t₂\| ≠ \|a₁−t₁\| + \|a₂−t₂\|`. More than one attribution is admissible on the text and **none excludes the others**, so this is **marked ratified rather than dressed as derivation**, on the `M35`/`M49`/`M50` precedent. **Derived — the key.** `§4.4(a)` already joins the truth side by *"`source_entity_id` belongs to a covered observation"*; `§16` fixes that field as *"the identifier of the observation whose obligation the posting records"* and names the agent-side counterpart *"named identically so that the two journals join structure to structure"*; `§12`/`M28` fixes the observation↔identifier relation and its **one-to-one** property on a conforming dataset, `DUPLICATE_ROW` being scoped to `bank_line` so *"no `entity_id` maps to two observations"*. **Ratified — the agent-side restriction.** `§4.4(a)` keys `proj_agent` by *"whose owning decision is `RECONCILED`"* and applies **no** `source_entity_id` predicate to it; restricting it by the same key is what makes the two sides comparable at one case, and is **adopted here rather than read off `§4.4`**. **Derived — the structural zero.** `§4.4` states *"(REFERENCE observations post nothing and enter none of these sets)"* and `§10.1` gives `payment` and `order` *"never posts a journal line, never enters a coverage numerator or denominator"*; `§16`'s grammar admits no `order_…`, and `§17.1.1` already reasons from that grammar to *"truth posts no line attributable to either kind"* for `ledger_entry` and `dispute`. Such a case is not covered and moves no account, so its harm is `0` **by the frozen text rather than by exclusion** — which is why it **stays in the denominator**: dropping it would narrow `M52`'s population, and `§4.8` requires the opposite, *"measuring it anyway is the point"*. **Rejected — the leave-one-out marginal:** attributing the aggregate by recomputing `balance_harm_inr` with `o` removed from the covered set is admissible on the text and is **not** the definition, because it makes one case's figure a function of every other case's — so the number would not be a property of the injection — and `§4.4`'s cancellation would let a genuine per-case harm read `0`. **Rejected — substituting `§4.4(b)`:** `misdirected_value_inr` is natively per-entity and would need no decomposition, but `§4.8` names `balance_harm`, and `§4.4` keeps the two *"reported separately"* because *"a system can be good at one and bad at the other"*; substituting one would answer `§4.4(b)`'s question under `§4.4(a)`'s name. Both rejections are **preserved as rejected** and neither may be adopted without reopening this row. **Disclosed:** `PREREGISTRATION.md §10` **V30** — the per-case figures do not sum to the published `balance_harm_inr` and **no additivity between them may be claimed or implied**. **Unchanged:** `M52`'s `injected` and `matched clean control` verbatim, including its TEST-only scope and *"population, not bijection"*; metric **16**'s formula and both its populations; `§4.4`'s own two figures and `§4.8`'s formulas; `PREREGISTRATION.md §8`'s list at **28**, none added, removed or renumbered; `RunKey` `(agent_id, split, seed, llm_mode)` and `RunConfig`; `§18`'s manifest **shape**; every `§7` threshold, `C1`–`C8` so `constraint_set_hash` does not move, `SE1`–`SE5`, `τ`, `ε`, and the oracle's labels and `§5.4`'s ambiguity definition, which this row never reaches. **`BENCHMARK_VERSION` moves 1.0.9 → 1.0.10** on `M39`'s precedent — an input to a figure on `§8`'s list enters the pre-registered surface — and **not** on `M49`'s, no conforming agent's postings changing; `GT_VERSION` stays **1.1.0**. **Ratified before the figure exists, which is the condition that makes it legitimate:** `bench/` absent, `runs/` holding only `.gitkeep`, no dataset generated, no agent scored, no metric computed and no seal tag cut, so `§6.2` **AL3** and `DECISION_BRIEF.md §L.4` are satisfied rather than merely not engaged. **No implementation is taken here:** `packages/eval/src/metrics/robustness.ts` is untouched, metric 15 stays unwired, and the build follows per `DECISION_BRIEF.md §I` |
| M56 | **`PREREGISTRATION.md §6.2` `AL5` is an EMISSION rule, and the scorer reads ground truth under `--sealed`.** `AL5` — *"The CLI's `--sealed` flag refuses to print, log or write any ground-truth field; only aggregate metrics are emitted"* — governs what leaves the process, not what enters it. `PREREGISTRATION.md §9` step 7's `assay bench --sealed` therefore **reads** `bench/test/<seed>/ground_truth.jsonl` and emits only aggregate metrics, which is what `EVALUATION_SPEC.md §2`'s `score(agent output, ground truth, oracle labels) -> metrics.json` has required of every scored unit on **both** splits since it was written. `§5.3`'s access restatement — *"`AL5` withdraws that route under `--sealed`"* — is **narrowed to the two readers it was written against**, the `§5.3` completeness gate and the `§9` seal, both of which `§9` runs **unsealed**; the withdrawal is preserved for them by a **flag refusal** on `assay oracle` and `assay seal` rather than by a read refusal. **Three states stay distinct and the ruling turns on keeping them apart: (A) agent execution under `--sealed` is UNCHANGED** — `AL1`, `AL2`, `AL4`, `AL6` and `AL7` are untouched in substance and wording, and no agent, engine or oracle may read the artifact sealed or not; **(B) truth/evaluation computation AFTER the agent run may read it**; **(C) the emitted scored artifact carries AGGREGATES ONLY and no `GroundTruth` field**. The ruling is **general**: it governs every truth-dependent metric — **2**, **3**, **5**, **6**, **7**, **8**, **15**, **16**, **26**'s cost half and `§5.1`'s ε curve — and every future one, not metrics 15 and 16 alone (`PREREGISTRATION.md §5.3`, `§6.2`, `§8`, `§9`, `§10` **V31**, `EVALUATION_SPEC.md §2`, `ARCHITECTURE.md §10`, spec 1.4.34) | **Ratified — the reading of `AL5`, and the record says so.** Two readings stood in the corpus and one had to be selected. `§6.2`'s binding text is an **emission** rule: reading is none of print, log or write, and metrics 15 and 16 **are** aggregate metrics. The stronger **read**-withdrawal reading entered at spec 1.4.27 (**M43**) in `§5.3`, `ARCHITECTURE.md §10` and `DECISION_BRIEF.md §A.34`. Since more than one reading is admissible on the frozen text, this is marked **ratified rather than dressed as derivation**, on the `M35`/`M45`/`M49`/`M50`/`M55` precedent. **Derived — the emission reading is the only executable one, which is `M45`'s own test.** `EVALUATION_SPEC.md §2` puts `ground truth` inside `score(...)` for `{dev, test}`; `§9` step 7 is the **only** run that ever scores TEST; `§8` marks metrics 15 and 16 **TEST-only**; `§7`'s M53 entry names a *"TEST scoring"* event that *"READS this table"*; and `EVALUATION_SPEC.md §5.4` item 5 requires **every** metric on the list of 28 in the report while `§5.5` bars *"any number that does not exist in a committed run artifact"*. Under the read-withdrawal reading `§9` step 7 can satisfy none of these. `M45` selected between two definitions of *"the seal"* on exactly this ground — *"the tag reading is selected because it is the only one under which this procedure's own steps 2 through 5 are executable"* — and the same test selects here. **Derived — the scorer was never among `AL1`/`AL2`'s constrained parties, so no permission is created.** `AL2` binds *"neither engine nor oracle code"* and `AL1` binds those same two packages' imports, **by name and not by category**; `§1`'s exclusion paragraph names engine, oracle, agent, baseline and ablation. `packages/eval/src/truth.ts` has recorded the consequence since it landed: *"Neither rule binds the scorer, and neither could … A scorer that could not see the answer key could not mark the paper."* What M56 removes is a `§5.3` sentence about two **other** readers that had come to bind a third; it grants nothing `AL2` had withheld. **Derived — why `§5.3`'s sentence is narrowed and not deleted.** It was written when zone `GENERATOR_TRUST` held exactly two readers, the `§5.3` completeness gate and the `§9` seal, **neither of which `§9` ever runs sealed** — step 3 and steps 4–5 carry no such flag. Its conclusion, *"neither gate runs sealed"*, is **true and is preserved**. Its premise had silently widened when the scorer became a third reader at spec 1.4.33. **Ratified — the gates' withdrawal is re-grounded on a flag refusal, which is stricter.** `assay oracle` and `assay seal` refuse `--sealed` outright rather than tripping a read guard, so the guarantee cannot be reached by a gate call site that happens to open the file. This is the property `DECISION_BRIEF.md §A.31` required when it rejected widening a shared zone: it must not rest *"on the fact that no gate call site happens to use it today"*. **Rejected, and preserved as rejected — a fifth `ReadZone` for the scorer.** `§A.31` multiplied a zone only to keep **two** readers of one artifact apart where a frozen rule kept them apart in words; here `AL2` already names its constrained parties by package, so a zone added to express what `AL2`'s own sentence expresses would be ceremony, and a fourth party in a four-zone table would invite the next reader to add a fifth. The scorer reads in `GENERATOR_TRUST`, the route `AL2` has always permitted. **Rejected, and preserved as rejected — a second scoring pass, or an unsealed step 7b.** `§9` step 8 reads *"NO CODE CHANGES BETWEEN 6 AND 8"*, and `M48` already fixed the consequence that *"re-rendering must not require re-scoring"*; `EVALUATION_SPEC.md §2` puts `score(...)` inside the per-agent loop, not after it. Adding a pass would change the official procedure silently, which is the failure this register exists to prevent. **Rejected, and preserved as rejected — copying or re-keying `ground_truth.jsonl`.** A second path to the artifact is a second evidence path, which `AL8` refuses in terms for the recon report and which `§A.31` refused again for the seal; and a copy outside `bench/<split>/<seed>/` falls outside `.gitignore`'s hold-back and outside `§9` step 4's digest, so the committed hash would no longer cover every instance of the bytes. **Rejected, and preserved as rejected — emitting `0.0` for an unavailable truth-dependent metric.** `EVALUATION_SPEC.md §5.5` bars *"any number in the demo that does not exist in a committed run artifact"*, and `M50` fixed the rule that *"an expectation the frozen text cannot admit is withdrawn, not reported"*. A zero standing in for an unread population is a fabricated number, and `§4.8` requires the opposite — *"measuring it anyway is the point"*. None of these four may be adopted without reopening this row. **Disclosed:** `PREREGISTRATION.md §10` **V31** — the guarantee now rests on an emission boundary rather than a read refusal, and the residual is stated there rather than argued away. **Unchanged:** `AL1`, `AL2`, `AL3`, `AL4`, `AL6`, `AL7` and `AL8` in substance **and in wording**, `AL7`'s burn rule and fail-closed default included; `AL5`'s own `§6.2` text, which is **read and not rewritten**; `§9`'s eight steps in number, order, command and flag, **step 0** included; `§1`'s `GroundTruth` in every field, type and comment, so `GT_VERSION` stays **1.1.0**; `PREREGISTRATION.md §8`'s list at **28**, none added, removed, renumbered or redefined, and **no formula, universe or threshold changes**; `§7` gains no entry and revises none; `M51`'s sweep grids, `M52`'s two populations, `M53`'s baseline and `M54`'s non-computable disposition; `C1`–`C8` so `constraint_set_hash` does not move; `SE1`–`SE5`, `τ`, `ε`; `RunKey` `(agent_id, split, seed, llm_mode)` and `RunConfig`; `§18`'s manifest **shape**; and the oracle's labels, which this row never reaches. **`BENCHMARK_VERSION` moves 1.0.10 → 1.0.11** on `M39`'s precedent — the pre-registered surface changes in what the sealed run can yield for nine figures on `§8`'s list — and **not** on `M49`'s, no conforming agent's postings changing. **`M45`'s non-bump is distinguished, not overlooked:** that row governed *when the test dataset becomes reachable* and changed nothing about a scored artifact's contents, whereas this row decides whether step 7 yields a **number** or a *"not exercised"* state. **Ratified before any figure exists, which is the condition that makes it legitimate:** `bench/` absent, `runs/` holding only `.gitkeep`, no dataset generated, no agent scored, no metric computed and no seal tag cut, so `§6.2` **AL3** and `DECISION_BRIEF.md §L.4` are satisfied rather than merely not engaged. **No implementation is taken here:** `apps/cli/src/fs/guard.ts`, `apps/cli/src/bench/scorer.ts`, `apps/cli/src/commands/bench.ts`, `packages/eval/src/truth.ts` and `packages/eval/src/metrics/robustness.ts` are all untouched, and the build follows per `DECISION_BRIEF.md §I` |
| M57 | **Metric 7 `ece`'s population, binned prediction and correctness predicate.** `EVALUATION_SPEC.md §4.6` fixed the formula, the ten equal-width bins, the reliability diagram and the ε-gap scope, and stated **no correctness source for `accuracy(bin)`**. It is supplied here and **the formula is not touched**. **Population:** the scored unit's **committed decisions carrying a non-null score** — `RECONCILIATION_SPEC.md §6` step 3's **`DISCRIMINATED`** branch, the one accept in which the ε-gap decided the gate. **Prediction:** **one committed decision is one prediction**, so `N` counts gate events. **Binned value:** `Δs = |evidence_score_bps(best) − evidence_score_bps(second)|` in integer basis points, the quantity `§13` carries as `AmbiguityCertificate.evidence_score_gap_bps`. **Correctness:** `assert(d) = { (d.target_id, e) : e a member entity of d }`, `truth(d) = { (target_id, entity_id) : a true allocation edge whose target_id = d.target_id }`, and `correct(d)` **iff `assert(d) = truth(d)` — set equality**, which is `M35`'s **allocation identity** applied as a comparison rather than as a sort key. A **strict subset** of the true member set is incorrect, a **superset** is incorrect, a target the truth carries no edge for gives `truth(d) = ∅`, and an asserted **empty** allocation is correct only against a true empty one. Correctness is against the **full true member set for the decision's target**, never against individual truth edges. **Bins:** ten of 1000 bps over the **full** `0..10_000` range, never the observed range; lower edge inclusive and upper exclusive except the tenth, which includes `10_000`; an empty bin contributes no term; **`N = 0` publishes the metric UNAVAILABLE with its reason and never `0.0`**. **`EVALUATION_SPEC.md §4.2` is read and NOT substituted**, and its abstained/excepted `FN` exclusion is **not imported** (`EVALUATION_SPEC.md §4.6`, `§5.4` item 5, `PREREGISTRATION.md §7`, `§8`, `§10` **V32**, spec 1.4.35) | **Ratified — the correctness predicate, and the record says so.** `§4.6` names `accuracy(bin)` and defines it nowhere, and at least two readings stood on the frozen text: **set equality** against the true allocation, which `RECONCILIATION_SPEC.md §6`/`M35` already calls *"allocation identity — the set of `(target_id, member_obs_id)` pairs the solution asserts"*, and **edge-level agreement**, under which `EVALUATION_SPEC.md §4.2`'s `FP` clause alone would decide and a decision asserting a subset of the true members would carry no false positive. The two **disagree numerically** on exactly that case, so the choice is outcome-bearing and is marked **ratified rather than dressed as derivation**, on the `M35`/`M49`/`M50`/`M55`/`M56` precedent. **Derived — the unit is the decision.** Every score in this corpus is a property of a `Candidate`: `§11` says `evidence_score_bps` *"orders candidates and feeds the ε-margin ambiguity test"*, and a `Candidate` is `(target_id, member_obs_ids)` — a whole allocation. **No frozen field carries a per-edge score**, and `§6`'s gate fires once per decision. An edge unit would replicate one gate event into as many predictions as the allocation has members and weight `n_bin / N` by allocation size — the unit confusion `§4.2` warns against (*"a settlement with 40 constituents is one record and forty independent claims"*) running in the other direction; `§4.2` chose the edge for a **set-membership** metric, while metric 7's unit is fixed by what carries the score. **Derived — the population.** `§6` step 3 tests materiality **first**, so a `UNIQUE` decision has no second solution and therefore **no `Δs` to bin** — inventing one is the fabricated number `EVALUATION_SPEC.md §5.5` bars — and an `IMMATERIALLY_AMBIGUOUS` decision was decided by the materiality clause, whose `§6.1` rationale is that *"the ledger is identical either way"*, so the score influenced nothing and `§4.6`'s *"the one place a soft score influences the gate"* does not reach it. An `AMBIGUOUS` decision abstains, `§13`'s `chosen_candidate_id` being `null`, so there is no committed allocation for `accuracy(bin)` to test and `EVALUATION_SPEC.md §4.3`'s metric 4 owns it; `INTRACTABLE` commits nothing. `DISCRIMINATED` is what remains. **Derived — the binned value.** `§4.6`'s own purpose is that *"an uncalibrated score cannot justify a **threshold**"*, and the only threshold this corpus applies to a score-derived quantity is **ε**, which `§6` compares against `Δs` and against nothing else. **Ratified — the exclusion of `§4.2`'s abstained/excepted parenthesis.** That parenthesis exists to prevent **cost double-counting** — *"`§4.5` already prices that decision at `C_review` or `C_exception`"* — and metric 7 prices nothing, so the rationale does not reach it; importing it would make one decision's correctness a function of the agent's **other** decisions, so two agents asserting an identical allocation against identical truth could be scored differently, which defeats the cross-agent comparability `EVALUATION_SPEC.md §2`'s protocol is built on. **Rejected, and preserved as rejected — calibrating `evidence_score_bps` itself instead of `Δs`.** `ε` is never compared against it, so the reading would calibrate a quantity no frozen threshold is applied to and justify no threshold at all. **Rejected, and preserved as rejected — including `UNIQUE` decisions with an invented score**, which `§5.5` forbids, and **including `IMMATERIALLY_AMBIGUOUS` decisions**, whose gate consulted no gap. **Rejected, and preserved as rejected — edge-level / partial-credit correctness**, which is `§4.2`'s `FP` clause with its `FN` clause deleted: `§4.2` pairs them because *"how much does it miss"* is a separate question, so that reading would calibrate `match_precision` and duplicate metric 5's numerator, while the claim the gate actually makes is that **this allocation explains this target** — what `§6` step 1 accepts, what `S5` validates and what `§17.1` posts. **Rejected, and preserved as rejected — the edge as the prediction unit**, for the weighting reason above. **Rejected, and preserved as rejected — leaving the metric unresolved.** `EVALUATION_SPEC.md §5.4` item 5 requires every metric on `PREREGISTRATION.md §8`'s list of **28** in the report; the gap was in `§4.6`'s own text rather than in the population, and **`M54`'s disposition does not transfer** — metric 10 has no truth axis, while metric 7 has one whose definition was merely undetermined. None of these may be adopted without reopening this row. **Disclosed:** `PREREGISTRATION.md §10` **V32** — set-equality correctness is **not equivalent** to edge-level correctness and metric **5** remains the partial-credit metric; and because only score-consulting `DISCRIMINATED` decisions enter, **sparse or empty bins are a structural property of the population** and are never grounds for changing this definition. No frequency, population size or expected value is claimed here or anywhere else: **no dataset exists**. **Unchanged:** `§4.6`'s formula `ECE = Σ_bins (n_bin / N) × | accuracy(bin) − mean_score(bin) |`, its **ten** equal-width bins and its reliability diagram; metric 7's **number and name**; `PREREGISTRATION.md §8`'s list at **28**, none added, removed, renumbered or redefined, and **no other metric's formula, universe or threshold moves**; `EVALUATION_SPEC.md §4.2` entire, `§5.4`'s **thirteen** obligations and `§5.5`'s forbidden practices; `§13`'s `Decision` and `AmbiguityCertificate` and `§11`'s `Candidate` in every field, type and comment; `§1`'s `GroundTruth`, so `GT_VERSION` stays **1.1.0**; `M51`'s sweep grids, `M52`'s two populations, `M53`'s baseline, `M54`'s non-computable disposition, `M55`'s per-case harm and `M56`'s three states; `C1`–`C8` so `constraint_set_hash` does not move; `SE1`–`SE5`, `τ`, `ε`, `K_max`, `C_max`, `P_max` and every other `§7` threshold; `RunKey` `(agent_id, split, seed, llm_mode)` and `RunConfig`; `§18`'s manifest **shape**; and `§9`'s eight steps in number, order, command and flag, **step 0** included. **`BENCHMARK_VERSION` moves 1.0.11 → 1.0.12** on `M39`'s precedent — the pre-registered surface changes in what the sealed run yields for a figure on `§8`'s list, metric 7 moving from an unavailable state to a number — and **not** on `M49`'s, no conforming agent's postings changing. **`M50`'s non-bump is distinguished, not overlooked:** that row **withdrew** two expectations and changed nothing a scored artifact contains, whereas this row decides whether metric 7 is a number at all — which is `M56`'s own test, applied to one metric instead of nine. Under-bumping is the error `M46` exists to make unrepeatable, so `§9` step 1's tag and step 5's literal are carried in this same amendment. **Ratified before any figure exists, which is the condition that makes it legitimate:** `bench/` absent, `runs/` holding only `.gitkeep`, no dataset generated, no agent scored, no metric computed and no seal tag cut, so `PREREGISTRATION.md §6.2` **AL3** and `DECISION_BRIEF.md §L.4` are satisfied rather than merely not engaged. **No implementation is taken here:** `packages/eval/src/metrics/calibration.ts` is untouched and stays correct for the predictions it is given, `apps/cli/src/bench/scorer.ts` and `apps/cli/src/artifacts/metrics.ts` still carry the standing-refusal constant this row supersedes, and the build follows per `DECISION_BRIEF.md §I` |
| M58 | **Metric 17's baseline encoding, and `PREREGISTRATION.md §7`'s authority over its source transcription.** `M53` fixed metric 17's rate, population, statistic, scope, producer, consumer and table and left **two** outcome-bearing choices open; both are ratified here and **no formula, universe, population or threshold moves**. **(1) ENCODING AND ROUNDING.** `§7` named the fields `mean_bps` / `stddev_bps` and stated **no rounding rule**. Frozen: the pair is **integer basis points**; the five per-seed rates enter the mean and the **sample** standard deviation at **FULL PRECISION** and are **not** rounded first; each statistic is converted to bps and rounded **EXACTLY ONCE**, at the end of `PREREGISTRATION.md §9` **step 0**'s arithmetic, by **`round_half_up` with ties away from zero**; the two figures are rounded **INDEPENDENTLY**, each from its own full-precision result, `stddev_bps` never being re-derived from `mean_bps` or from rounded inputs; and the detector consumes the **ROUNDED** pair — `abstention_spike_flag = rate > mean_bps / 10_000 + k_sigma * stddev_bps / 10_000` — against the run's own **FULL-PRECISION** rate, `k_sigma` **unchanged at 3**. **NO SECOND, UNROUNDED BASELINE EXISTS ANYWHERE IN THE SYSTEM.** **(2) RECORD AUTHORITY AND TRANSCRIPTION.** `PREREGISTRATION.md §7`'s metric-17 table is the **AUTHORITATIVE human-readable** baseline record; `METRIC_17_BASELINE` in `packages/eval/src/frozen.ts` is its **EXECUTABLE TRANSCRIPTION**, intentionally **empty (`[]`) before step 0**; after step 0 measures the five DEV rates the exact `(agent_id, llm_mode, mean_bps, stddev_bps)` rows are transcribed into **both**, **after step 0 and BEFORE `§9` step 1's tag**, so the tag covers the source carrying the measured baseline; the constant is **not** a second independently measured baseline; **no runtime scoring may recompute the baseline**; **any divergence between the two is a SEAL/REPRODUCIBILITY FAILURE**; **nothing may be guessed, prefilled or populated before step 0**; **no generated JSON or data file** is introduced as another evidence path; and the constant is **not** a `BenchmarkManifest` field, `§18`'s shape staying **closed** (`PREREGISTRATION.md §7`, `§8`, `§9`, `§10` **V33**, `EVALUATION_SPEC.md §4.10`, `§0` rule 5 read and not amended, spec 1.4.36) | **Ratified — both choices, and the record says so.** Each admitted more than one reading on the frozen text and the readings **disagree numerically**, therefore on `abstention_spike_flag`; that is the condition under which this corpus marks a choice **ratified rather than dressed as derivation**, on the `M27`/`M35`/`M45`/`M49`/`M50`/`M55`/`M56`/`M57` precedent. **Derived — the integer encoding, from `§0` rule 5's own words.** Rule 5 binds *"every dimensionless ratio that enters a hashed body, **or that a gate or invariant compares**"*, and this pair **is** the right-hand side of `EVALUATION_SPEC.md §4.10`'s detector. Rule 5's carve-out cannot be reached on two independent grounds: it is **enumerated and closed** — *"the four `coverage_by_value*` figures and `coverage_by_count` in `CloseReport` (§20)"* — and it is **conditioned** on being *"derived display values computed at render from the authoritative integer paise fields"*, while metric 17's baseline is a statistic over five prior runs that no longer exist as artifacts when the flag is evaluated, so there is nothing at render for it to be derived **from**. Widening an enumerated carve-out is the move `M56` refused in terms. **Derived — the `_bps` suffix already means this.** Rule 5's own list of covered fields is `evidence_score_bps`, `evidence_score_gap_bps`, `epsilon_bps` and `max_unresolved_ratio_bps`, every one an integer a gate compares; and `§21`'s `AbstentionTelemetry` already carries the **ratio** spelling for the same quantities as `baseline_rate_by_value` and `baseline_stddev`, *"echoed here, never computed per run"*. Both spellings and both roles already exist in this corpus; the rejected reading would have inverted them. **Ratified — rounding once at the end, over full-precision internals with `_bps` as display.** The rejected reading does not remove the rounding decision, it **relocates** it into the `§7` transcription and leaves it **unspecified** there, a markdown table being unable to carry a double without a decimal-place convention; and it would make this row's own companion rule — *"any divergence between `§7` and `METRIC_17_BASELINE` is a seal failure"* — **unenforceable**, no exact comparison existing between a truncated rendering and a full-precision literal. A ratification that renders its companion uncheckable is refused. **Derived — the source constant must be exactly comparable.** A `number` is an IEEE-754 double, so *"full precision"* in a source literal means *precision-to-17-digits-then-parse*, not exactness; a constant whose legitimacy rests on being **identical** to a document row must be exactly representable, which integers in `0..10_000` are and decimal fractions of a ratio are not. The alternative turns every document-versus-source check into a tolerance comparison, and *"adding a tolerance to `C6`"* is on `DECISION_BRIEF.md §L.4`'s prohibited list. **THIS IS METRIC 17'S RULE AND NOTHING WIDER, and the register states it because the implementation's docstring currently claims otherwise.** It is **NOT** a claim that half-up is this corpus's only rounding or quantization mode. `M27` ratified a **floor** for `mode_days`; `EVALUATION_SPEC.md §4.6`'s bin selection **floors**; remainder distribution **floors**. Those rules are **untouched and not reopened**. What this row takes from `M27` is its **structure**, not a mode: `M27` quantized the recorded, compared term (`mode_days`) and left the continuous term continuous (`lag_days`, *"the **unfloored** real quotient"*), scoping the rationale *"to the mode alone"* — and this row quantizes the recorded baseline pair while leaving the run's own rate at full precision. **Derived — `§7` is the authority and the source is a transcription, not a second measurement.** `§7`'s consumer row already says *"TEST scoring READS this table. No baseline is computed at scoring time on any split"*; `packages/eval/src/frozen.ts`'s own header already declares itself *"the single place those parameters enter this package"*; and `CONSISTENCY_DRAW_SEED` is the standing precedent — a `§7` value no frozen rule derives, fixed at a governance gate **before any result existed**, carried as a source constant with the document as record. **Ratified — the transcription is ordered inside step 0, before the tag.** `§7` requires the baseline *"computed before the seal"* and `§9` step 8 bars code changes once the seal point is recorded, so a constant written **after** the tag would put a measured figure outside the seal. **Rejected, and preserved as rejected — a generated file.** It is a second evidence path for a figure `§7` alone is authoritative for, which `AL8` refuses in terms and `DECISION_BRIEF.md §A.31` and `M56` refused again; regenerating it after step 6 is a code change step 8 bars, while committing it makes it a source constant with an unaudited generator interposed; and a generator implies the value is **re-derivable**, reopening the door `§7`'s consumer row closes. **Rejected, and preserved as rejected — a runtime JSON or data file read by the scorer.** `ARCHITECTURE.md §3` gives `apps/cli` all filesystem I/O, and a scorer that opened a file for its baseline could be pointed at a different file; a source constant is covered by step 1's tag. **Rejected, and preserved as rejected — a `BenchmarkManifest` field**, which `M53` already rejected and which this row does not revisit; and **parsing `PREREGISTRATION.md` at runtime**, which would make a document a program input. **Rejected, and preserved as rejected — a `false` flag or a zero baseline where `§7` records no row.** `M56` refused `0.0` for an unavailable metric and `M57` carried the rule forward; a detector reporting *"no spike"* against a baseline it does not have is the broken detector `EVALUATION_SPEC.md §4.10` names, and the flag is published **UNAVAILABLE with its reason**. None of these may be adopted without reopening this row. **Clarified, not amended — the command surface and step 0's invocation.** `assay bench --baseline` is the **implementation spelling** of step 0's `<non-scored DEV baseline pass>`, which names no command: a **mode of the existing `bench` command, not a ninth command**, `DECISION_BRIEF.md §C` **T0-11**'s list of **eight** staying unchanged and unedited on `M43`'s rule that *"the command surface is the frozen text's own, and nothing is invented"*. `§9` step 0's *"under each `llm_mode`"* constrains the **completeness of `§7`'s table** — a row per `(agent_id, llm_mode)` — and **not the number of process invocations**, step 0 already being written as three command lines; the operator may take it **once per `llm_mode`**. **No step is added or renumbered and no second benchmark procedure is created.** Where `DECISION_BRIEF.md §F` **F2** is unresolved the `replay` rows are deferred exactly as **F2** already defers `B2-LLM-DIRECT` and metric 3's `--llm=replay` column; **F2's semantics are applied, not reopened.** **Disclosed:** `PREREGISTRATION.md §10` **V33** — a full-precision rate is compared against a quantized bar, so a rate within roughly **2 bps** of the true bar may fall on the other side of the flag, and a genuinely non-zero `σ` below `0.5` bps records as `0`. **Unchanged:** `EVALUATION_SPEC.md §4.10`'s formula `rate > baseline + k·σ` **verbatim** and `k_sigma = 3`; metric 17's **name and number**; `M53`'s rate, numerator, denominator, five-seed population, sample statistic, `(agent_id, llm_mode)` scope, producer and consumer; `PREREGISTRATION.md §8`'s list at **28**, none added, removed, renumbered or redefined, and **no other metric's formula, universe or threshold moves**; `§0` rule 5, `§18`'s manifest **shape**, `§21`'s `AbstentionTelemetry` in every field, type and comment, and `§1`'s `GroundTruth`, so `GT_VERSION` stays **1.1.0**; `M1`, `M27`, `M35`, `M42`, `M46`, `M49`, `M51`–`M57` entire; `C1`–`C8` so `constraint_set_hash` does not move; `SE1`–`SE5`, `τ`, `ε`, `K_max`, `C_max`, `P_max`, `C_review`, `C_exception`, `queue_top_n` and every other `§7` threshold; `RunKey` `(agent_id, split, seed, llm_mode)` and `RunConfig`; `AL1`–`AL8` in substance **and** wording; and `§9`'s **eight steps in number, order, command and flag**, **step 0** included. **`BENCHMARK_VERSION` moves 1.0.12 → 1.0.13** on `M39`'s precedent — the pre-registered surface changes in what the sealed run can yield for `abstention_spike_flag` and the echoed baseline pair, the two readings disagreeing numerically — and **not** on `M49`'s, no conforming agent's postings changing. **`M50`'s and `M45`'s non-bumps are distinguished, not overlooked:** `M50` withdrew expectations and changed nothing a scored artifact contains, and `M45` governed *when* the test dataset becomes reachable, whereas this row changes an artifact's **contents**. Under-bumping is the error `M46` exists to make unrepeatable, so `§9` step 1's tag and step 5's literal are carried in this same amendment. **This row REVISES a `§7` entry, which `M56` and `M57` did not, and that is legitimate only because nothing has been observed:** `DECISION_BRIEF.md §L.4` bars changing a `§7` parameter *"on the basis of an observed result"* and `PREREGISTRATION.md §6.2` **AL3** permits pre-seal adjustment *"only on an argument that does not reference measured performance"* — and **`§9` step 0 has not run**, so no rate, mean or `σ` exists. The entry specifies **how a not-yet-existing value is formed** and changes **no value**. **Ratified before any figure exists, which is the condition that makes it legitimate:** `bench/` absent, `runs/` holding only `.gitkeep`, no dataset generated, no agent scored, no metric computed, **no baseline measured**, `METRIC_17_BASELINE` empty and no seal tag cut. **No implementation is taken here:** `packages/eval/src/metrics/abstention.ts`, `apps/cli/src/bench/baseline.ts`, `apps/cli/src/bench/scorer.ts`, `apps/cli/src/commands/bench.ts` and `apps/cli/src/artifacts/metrics.ts` are untouched in behaviour, the baseline algorithm is unchanged, and the build follows per `DECISION_BRIEF.md §I`. **One runtime follow-up is chartered:** `packages/eval/src/metrics/abstention.ts` currently asserts *"Half-up is this corpus's **only** rounding mode"* and that *"nothing in the corpus uses another"*. **That statement is FALSE** — `§4.6`'s bin selection floors, `M27`'s `mode_days` floors, remainder distribution floors — and it is currently offered as the **justification** for the rule this row ratifies. The runtime `M58` commit **must** replace it with wording grounded in this metric-17-specific ratification and **must** state explicitly that other rules in this corpus use other rounding and quantization modes |
| M59 | **`PREREGISTRATION.md §9` step 0's measured metric-17 baseline is `(0, 0)` for every `offline` Tier-0 key, and `EVALUATION_SPEC.md §4.10`'s detector is read against it unchanged.** Step 0 has been taken and returned `mean_bps = stddev_bps = 0` for `ASSAY`, `B0-IDONLY`, `A1-NOVALIDATE`, `A2-NOABSTAIN` and `A3-NOLLM` under `llm_mode = offline`. `M53` fixed metric 17's rate, population, statistic and producer and `M58` fixed the pair's encoding and the `§7` ↔ `METRIC_17_BASELINE` record relationship; **neither addressed what the detector publishes when the measured pair is `(0, 0)` for every agent**, and that question is settled here. **The result is ENTAILED by structure this register already carries and is NOT a newly discovered property.** `PREREGISTRATION.md §10` **V17** records, before the seal, that *"the oracle's candidate-search machinery is exercised on no DEV target"* because *"every DEV settlement is fully `AN1`-anchored: `F08`'s `DROP_SETTLEMENT_ID` is the only operator that detaches a line from its batch identifier, and `F08` is **test-only** at seeds 9100–9104 (`§6.1`)"*, closing with *"the completeness gate passes on DEV without ever enumerating a candidate"*; **V28** records that `§6.1` makes `F07`–`F10` test-only, so *"no DEV dataset can contain the adversarial family"*. A population on which no candidate is enumerated presents no target `RECONCILIATION_SPEC.md §6` can mark `AMBIGUOUS` or `INTRACTABLE`, so `§9`'s abstention branch is not reached, no `recon_line` reaches `ABSTAINED`, and `§4.10`'s numerator is zero on every seed for every agent. `A2-NOABSTAIN` is zero on a second and independent ground — it is the agent `EVALUATION_SPEC.md §3.2` defines as never abstaining — and that ground is `V33`'s and is unchanged | **Ratified — one choice, and the record says so.** The frozen text admitted more than one reading of what `§4.10` publishes when `§7` carries a measured `(0, 0)` row, and the readings **disagree in the artifact**: the flag is a boolean under one and `null` under the other. That is the condition under which this corpus marks a choice **ratified rather than dressed as derivation**, on the `M27`/`M35`/`M45`/`M49`/`M50`/`M55`/`M56`/`M57`/`M58` precedent. **What is ratified:** a measured `(0, 0)` pair **is** a baseline for `§4.10` and `§21`, so `metric17BaselineFor` returns it, the flag is **computed** and reported, and `§5.5`'s *"unavailable with its reason"* is **not** engaged — that convention governs a key `§7` records **no pair** for, which is a different condition from a key whose measured pair is zero. **The detector consequence, stated arithmetically and not as a change:** on a baseline of `(0, 0)` the frozen expression `rate > mean_bps / 10_000 + k_sigma · stddev_bps / 10_000` evaluates to `rate > 0 + 3 · 0`, i.e. `rate > 0`. The comparison being strict, a scored run carrying no abstained `recon_line` value yields `false` and one carrying any positive abstained `recon_line` value yields `true`. **What the published flag therefore identifies for such a key is the presence of any positive abstained `recon_line` value, not an excursion above a non-degenerate reference**; the word *spike* in the field's name is read with this row attached. `§4.10`'s stated expectation is met on its own terms — *"expected to fire on the F10 adversarial split and not to fire on clean splits; a flag that fires everywhere is a broken baseline, and one that fires nowhere is a broken detector"* — because a clean run does not fire and an `F10` run carrying abstained value does; **no frequency, magnitude or expected firing rate is asserted**, no scored run existing. **`V33` is not edited and its instance is not withdrawn:** V33 ratified the `σ = 0` collapse and named `A2-NOABSTAIN` *"the declared instance"*; this row records a **second, distinct instance** — a zero σ arising from a population that presents nothing to abstain over rather than from an agent that cannot abstain — and records that under it the **mean** is also zero, a case V33's phrasing *"the bar collapses to `mean_bps`"* does not describe. **Unchanged:** `§4.10`'s formula `rate > baseline + k·σ` **verbatim**; `k_sigma = 3`; metric 17's **name and number**; `M53`'s rate, numerator, denominator, five-seed population, sample statistic, `(agent_id, llm_mode)` scope, producer and consumer; `M58`'s encoding, rounding and transcription rules; `PREREGISTRATION.md §8`'s list at **28**, none added, removed, renumbered or redefined; `§6.1`'s split table, family assignments and forbidden list; `§4.2`'s and `§4.3`'s generator parameters; `RECONCILIATION_SPEC.md §3`'s `AN1` anchor rule; `packages/oracle`'s enumeration and `τ`; `§9`'s **eight** steps in number, order, command and flag; `RunKey`, `RunConfig` and `GroundTruth`; and **no other metric's formula, universe or population**. **Four alternatives are rejected and preserved as rejected:** reporting the flag UNAVAILABLE for a measured `(0, 0)` key, which would read `§5.5`'s absent-key convention onto a present measurement and leave `§4.10` publishing nothing on the sealed run; **re-running step 0**, the pass being deterministic in its inputs over unchanged datasets and the result entailed by V17, so a re-run returns identical values and consumes clean provenance for nothing; **widening the DEV population**, already rejected at V28 and barred by `§6.1`'s forbidden list under `AL7`'s burn rule; and **altering the detector, threshold or formula** — flooring the bar, imposing a minimum σ, relaxing `>` to `≥`, or moving `k_sigma` — barred by `§6.2` `AL3`, by `M58`'s verbatim preservation, by `DECISION_BRIEF.md §L.1` rule 12, and dispositively by `§L.4`, which *"forbids revising it from a result"*. **`BENCHMARK_VERSION` does NOT move, staying 1.0.13**, and `GT_VERSION` stays **1.1.0**: `M49`'s test is not met, no conforming agent's postings changing, and `M39`'s is not met either — that test asks whether the pre-registered surface changes in **what the sealed run can yield**, and the reading ratified here is the one the frozen implementation already realises, so nothing about the run's output moves; the reading **rejected** here is the one that would have moved it. `constraint_set_hash` is `packages/domain`'s and is unmodified. **No dataset is regenerated**: `bench/dev` is step 0's evidence, it embeds no benchmark version, and each oracle gate's `spec_version` stamp records the version under which that gate **ran** and correctly reads `1.4.36`. **This row is taken AFTER a measured figure exists, and it does not reuse the *"fixed before any figure exists"* formula `M55`–`M58` each close on, which is no longer available and would be false.** Its legitimacy is stated instead in five checkable parts: the figure came from the **already-frozen procedure**, executed as `§9` step 0 is written after a pre-flight verifying `METRIC_17_BASELINE` empty, no tag and no scored `runs/` artifact; the amendment **does not tune or revise the measured quantity**, the five rows being transcribed unchanged with nothing recomputed, re-rounded, re-scoped or re-attributed; it **records and ratifies a consequence of already-frozen structure** — V17, V28, `§4.10`'s verbatim formula and V33's σ disposition — every premise predating the measurement, and stating a conjunction of frozen premises is not revising them; **no scored TEST result has been observed**, no TEST seed generated or inspected, no `--sealed` run executed, no `metrics.json` produced and no flag evaluated on any scored unit, `AL4` and `AL7` being untouched and `runs/` holding only `.gitkeep`, the figure that exists being a **non-scored pre-seal DEV baseline** which `§9` step 0 declares emits no `metrics.json`, forms no `RunKey` and writes no `runs/` artifact; and **no detector threshold or benchmark parameter is altered**, so `AL3` and `§L.4`'s bar on revising the pre-registered surface **from a result** is not engaged by a row that changes no rule, no threshold, no population and no line of metric code. **That distinction is the whole of this row's claim to legitimacy, and it fails if any part of this amendment is later found to have moved a parameter.** The residual is `PREREGISTRATION.md §10` **V34** |
| M60 | **`DATA_MODEL.md §19`'s `LlmProviderId` gains a FIFTH member, `gemini`**, and `ARCHITECTURE.md §6.5` gains the matching row: `@google/genai` against the Gemini Developer API, `responseJsonSchema` for the strict schema, `systemInstruction` for the role prompt, `requiresNetwork: true` and `meteredCost: true`. The implementation lives at `apps/api/src/explain/gemini.ts` beside the `anthropic` one, `packages/llm` gains no transport, and the credential is read server-side from `GEMINI_API_KEY` with the model id from `GEMINI_MODEL` (`ARCHITECTURE.md §4`/`§6.5`/`§12`, `PROJECT_SPEC.md §6.1`, `DECISION_BRIEF.md §L.4`, spec 1.4.38) | **Derived — the fourth member could not carry it.** `§6.5` scopes `openai-compatible` to *"any endpoint speaking the OpenAI chat-completions schema with JSON-schema response format"*. The Gemini Developer API reached through `@google/genai` speaks Google's own `generateContent` schema and not that one, so the id was **available only on a false description of the transport** — and the field it would have been written into is `§19`'s `provider`, whose stated purpose is that *"a report can always state exactly what produced each decision"*. Two readings were open and one is foreclosed by that sentence: recording a native Google call as `openai-compatible` makes the provenance field describe a transport that was never used, which is the one thing `§19` exists to prevent. Widening is what is left. **Ratified — the position.** Appended fifth, never inserted: `§8`'s *"appended, never renumbered"* principle governs an ordered declaration for the same reason, and `apps/cli`'s `--llm` usage text reads the list in order. **Derived — nothing already written moves.** No committed artifact carries the new member; `LLM_PROVIDER_IDS` is read for **validation** and is not hashed, so no sealed event re-hashes; every scored run records `offline`. **`meteredCost` is `true` and the free tier does not change it**, because the flag describes the provider row and `apps/cli`'s `buildProvider` refuses on `meteredCost || requiresNetwork` — so no test run and no scored run can reach this provider, `§L.1` rule 10's `--llm=offline` guarantee is untouched, and `§C` T0-11's clean checkout with no API key still runs. **`§L.4` and `PREREGISTRATION.md §6.2` `AL3` are satisfied rather than merely not engaged**: this row adds no LLM call outside roles R1–R4 and none outside the `LlmProvider` interface, alters no frozen threshold, no decision parameter, no population, no metric and no line of scoring code, and **references no observed result** — it is a transport declaration taken for a cost reason stated in advance, not a revision made from a figure. **Benchmark stays v1.0.13 and `GT_VERSION` stays 1.1.0.** `§18`, `§20`, `§21` and `§1` are read and not amended; `M1`, `M27`, `M35`, `M42`, `M46`, `M49`, `M51`–`M59` are unchanged and preserved verbatim. **The residual is structural and carries no magnitude**: `R4` is the only role this provider serves, it is served from `apps/api` alone, and no benchmark population is defined over it — so this row supports no claim about how often the provider answers, how often it is refused, or how it compares with any other |
| M61 | **`RECONCILIATION_SPEC.md §6`'s materiality is UNDEFINED — not zero — on a target with no `AN2`-matched bank line, the `IMMATERIALLY_AMBIGUOUS` branch is evaluated only when materiality is defined, and `§13`'s `reason` gains a FIFTH member, `MATERIALITY_UNDETERMINED`, which takes precedence over the three attempts-derived reasons whenever an `AMBIGUOUS` outcome carries `materiality_paise: null`** | **Derived — from a rule this document already states.** `§17.1.1`: *"`I5` is undefined — not satisfied — when no bank-line mapping exists. With no mapping there is no right-hand side, so the comparison has no truth value"*, and the permissive reading *"produces exactly the failure `I5` names in its own purpose column"*. `§6`'s materiality runs *"both allocations through the ledger projection"*, and `§17.1.1` conditions the only allocation-dependent postings, `P2`/`P4`, on `AN2`; so on a target without an `AN2` line neither projection carries a bank leg, the difference has no comparand, and the same rule applies to the same absence. `s5-validate.ts` already represents it as `bank_tie_out: null ⇒ skip`; `s4-solve.ts` now represents it as `materiality_paise: null ⇒ the immateriality branch is not evaluated`. **The defect this corrects, stated as measured.** Through spec 1.4.38 `s4-solve.ts`'s `balances` returned an **empty** projection for `bank_evidence === null`, `materiality` computed the difference of two empty projections as `0`, `0 ≤ τ` held, and `solve` returned `IMMATERIALLY_AMBIGUOUS` — on which `apps/cli`'s `classifyTarget` **commits the top candidate**. `PREREGISTRATION.md §4.2` freezes `bank_ref` quality at 30 % clean, so this was the outcome on the majority of any multi-candidate population, while `PREREGISTRATION.md §5.4`'s oracle — whose `projectAllocation` posts `P2`/`P4` unconditionally — labelled the same target `TRULY_AMBIGUOUS`. An absent comparand was licensing a commit; `PROJECT_SPEC.md`'s thesis that under-determined evidence produces a typed abstention did not hold on that population. Recorded for the sealed run at `PREREGISTRATION.md §10` **V37**; the sealed artifact is not re-scored. **Ratified — the fifth reason rather than a fold into `EVIDENCE_TIE`.** The four existing reasons each answer *why did the search fail to separate two allocations*; this state answers *why was the immateriality escape hatch unavailable*, a different axis with a different human action — obtain the bank line, rather than adjudicate the evidence. Reporting it as `EVIDENCE_TIE` would state a materiality finding (`> τ`) that was never made. A second certificate field (`materiality_status`) was considered and rejected for this amendment: it would change `§16`'s hashed body key set, where the enum widening does not. **Blast radius, stated.** `packages/ledger` `CERTIFICATE_REASONS` (+1 member; `sealCertificate` admits it; `CERTIFICATE_KEYS` unchanged; `materiality_paise` keeps its non-nullable shape and reads `0` under this reason, as it already did under `SEARCH_BOUND_EXCEEDED`); `packages/engine` `s4-solve.ts` (`balances`/`materiality` return `null`; the ladder's first branch is guarded; `SolveResult.materiality_paise` is `null` on this path); `packages/controller` `CertificateReasonSchema` (+1); `apps/web` copy (the materiality tile and the story's "why" sentence read `reason` before printing a figure). **Unreached:** every sealed artifact — `runs/seal-v1.0.13` carries zero abstentions and therefore zero certificates, so no sealed body names a reason, and a wider enum parses every sealed body identically; `constraint_set_hash` (`C1`–`C8` untouched); `τ`, `ε`, `K_max`, `C_max`, `P_max`, `SE1`–`SE5` weights; `I1`–`I9`; the oracle, whose label was already correct. **Scope, and the test that pins it.** Fires only on `≥ 2` admissible candidates with `bank_evidence === null` and `Δs < ε`; a single candidate is `UNIQUE` and commits; `DISCRIMINATED` stays reachable with undefined materiality because the gap arm needs no comparand. `packages/engine/tests/s4-materiality-undetermined.test.ts` holds the regression test, which failed against the spec-1.4.38 engine before the fix, the two scope tests, and a property over generated components. **Versioning.** `SPEC_VERSION` moves **1.4.38 → 1.4.39**, and because `apps/cli` stamps `engine_commit: SPEC_VERSION` into every run's genesis (`§16`), a run produced by the corrected engine is distinguishable from a sealed one by its own record. `BENCHMARK_VERSION` stays **1.0.13**: the row changes what a conforming agent **decides** — the ground M49 moved the version on — but a bump here would name a sealed run that does not exist (`tests/discipline.test.ts` derives the sealed directory from the constant), and the corrected engine's first sealed run is bench-v2's, under fresh seeds, where the bump belongs. Runs either side of this row are **not comparable**, and V37 says so |
